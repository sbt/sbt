...
object FileInfo:
  ...
  given byteArrayFormat: JsonFormat[Array[Byte]] = new JsonFormat[Array[Byte]]:
    override def read[J](jsOpt: Option[J], unbuilder: Unbuilder[J]): Array[Byte] =
      jsOpt match
        case Some(js) =>
          try Hash.fromHex(unbuilder.readString(js))
          catch
    ...
    private[sbt] def fileOrDirectoryMax(file: File): ModifiedFileInfo =
      val maxLastModified =
        if file.isDirectory then
          FileTreeView.default.list(file.toGlob / **).foldLeft(0L) {
            case (max, (path, attributes)) =>
              val lm = if !attributes.isDirectory then IO.getModifiedTimeOrZero(path.toFile) else 0L
              if lm > max then lm else max
          }
