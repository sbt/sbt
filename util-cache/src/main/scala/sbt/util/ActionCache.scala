...
object ActionCache:
   ...
   * Avoids races from a direct `Files.copy` into `destZip` under parallel task execution. On
   * Windows the rename itself can be refused while a concurrent install of the same directory
   * holds `destZip` open, so it is retried and, as a last resort, loses to the concurrent zip.
   */
