...
object ActionCache:
  ...
  /**
   * Publishes `builtZip` as `destZip` by copying into place.
   * Uses `IO.copyFile` so that Windows-specific retry / sharing-violation
   * handling can be centralised in the IO layer rather than duplicated here.
   */
  private def installPackagedZip(builtZip: Path, destZip: Path, fallbackStagingDir: Path): Unit =
    val alreadyInstalled =
      Files.exists(destZip) &&
        Digest.sameDigest(destZip, Digest.sha256Hash(builtZip))
    if !alreadyInstalled then
      Option(destZip.getParent) match
        case Some(parent) => Files.createDirectories(parent)
        case None         => ()
      IO.copyFile(builtZip.toFile, destZip.toFile, true)
