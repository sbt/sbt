...
object ActionCache:
  ...
  /**
   * Publishes `builtZip` as `destZip` via `IO.copyFile`, which stages next to the destination and
   * retries the rename into place. On Windows the rename is refused while a concurrent install of
   * the same directory still holds `destZip` open; once retries are exhausted, its zip wins.
   */
  private def installPackagedZip(builtZip: Path, destZip: Path): Unit =
    try IO.copyFile(builtZip.toFile(), destZip.toFile())
    catch
      // Callers derive digests from the file on disk, so the surviving install
      // determines the recorded content either way.
  ...
  def packageDirectory(
    ...
    IO.withTemporaryDirectory: tempDir =>
      ...
      installPackagedZip(tempZipPath, zipPath)
