...
object Tracked {
  ...
  def lastOutput[I, O](cacheFile: File)(f: (I, Option[O]) => O)(implicit o: Format[O],
                                                                mf: Manifest[Format[O]]): I => O = in => {
  ...
  private[sbt] def lastOutputWithJson[I, O: Pickler: Unpickler](
      cacheFile: File
  )(f: (I, Option[O]) => O): I => O = in => {
  ...
  private[sbt] def inputChangedWithJson[I: Pickler: Unpickler, O](
      cacheFile: File
  )(f: (Boolean, I) => O): I => O = in => {
  ...
  def outputChanged[I, O](cacheFile: File)(f: (Boolean, I) => O)(implicit ic: InputCache[I]): (() => I) => O =
    in => {
      val initial = in()
      val help = new CacheHelp(ic)
  ...
  private[sbt] def outputChangedWithJson[I: Pickler, O](
      cacheFile: File
  )(f: (Boolean, I) => O): (() => I) => O = in => {
    val initial = in()
    val help = new JsonCacheHelp[I]
...
class Timestamp(val cacheFile: File, useStartTime: Boolean) extends Tracked {
  ...
  def readTimestamp: Long =
    try { read(cacheFile).toLong } catch {
      case _: NumberFormatException | _: java.io.FileNotFoundException => 0
    }
}
...
class Changed[O](val cacheFile: File)(implicit equiv: Equiv[O], format: Format[O]) extends Tracked {
  ...
  def uptodate(value: O): Boolean =
    try {
      Using.fileInputStream(cacheFile) { stream =>
        equiv.equiv(value, format.reads(stream))
      }
    } catch {
...
class Difference(val cache: File,
                 val style: FilesInfo.Style,
...
object FileFunction {
  ...
  )(action: Set[File] => Set[File]): Set[File] => Set[File] =
    cached(cacheBaseDirectory)(inStyle, outStyle)((in, out) => action(in.checked))

  def cached(
      cacheBaseDirectory: File
  )(inStyle: FilesInfo.Style, outStyle: FilesInfo.Style)(action: UpdateFunction): Set[File] => Set[File] = {
    import Path._
