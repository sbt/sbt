...
private[sbt] class BatchScriptRunner extends ScriptRunner with AutoCloseable:
  ...
  def apply(statements: List[(StatementHandler, Statement)], states: States): Unit =
    if states.isEmpty then super.apply(statements)
  ...
  def cleanUpHandlers(handlers: Seq[StatementHandler], states: States): Unit =
    for handler <- handlers; state <- states.get(handler) do
  ...
  def processStatement(handler: StatementHandler, statement: Statement, states: States): Unit =
    ...
    nextState match
      case Left(err) =>
        if statement.successExpected then
          err match
            case t: TestFailed =>
      ...
      case Right(s) =>
        if statement.successExpected then states(handler) = s
        else throw new TestException(statement, "Command succeeded but failure was expected", null)

  override def close(): Unit = ()
end BatchScriptRunner
