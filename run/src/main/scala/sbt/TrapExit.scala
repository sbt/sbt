...
private final class TrapExit(delegateManager: SecurityManager) extends SecurityManager {
  ...
  private final class App(val execute: xsbti.F0[Unit], val log: Logger) extends Runnable {
    ...
    private[this] def threadsInGroups(toProcess: List[ThreadGroup], accum: List[ThreadGroup]): List[Thread] =
      toProcess match {
        case group :: tail =>
...
/**
  ...
  */
private final class LoggingExceptionHandler(log: Logger, delegate: Option[Thread.UncaughtExceptionHandler])
    extends Thread.UncaughtExceptionHandler {
