...
private object sbtApplicationID extends xsbti.ApplicationID {
  ...
  override def version(): String =
    StaticUtils.getProperty(getClass.getClassLoader, "xsbt.version.properties", "version") getOrElse "unknown"
...
private class StaticLauncher(appProvider: StaticAppProvider, scalaProvider: StaticScalaProvider)
    extends xsbti.Launcher {
  override def getScala(version: String): xsbti.ScalaProvider = getScala(version, "")
  override def getScala(version: String, reason: String): xsbti.ScalaProvider =
    getScala(version, reason, StaticUtils.SCALA_ORG)
  override def getScala(version: String, reason: String, scalaOrg: String): xsbti.ScalaProvider = {
    ...
    if (myScalaVersion == version) scalaProvider
      ...
      throw new InvalidComponent(
        s"This launcher can only provide scala $myScalaVersion, asked for scala $version"
      )
  }
  ...
  override def appRepositories(): Array[xsbti.Repository] =
    Array(new FakeRepository(new FakeResolver("fakeresolver", bootDirectory / "fakeresolver-cache", modules)))
...
private class StaticScalaProvider(appProvider: StaticAppProvider) extends xsbti.ScalaProvider {
  ...
  def getComponent(componentID: String): File = {
    ...
    assert(
      component.length == 1,
      s"""Component $componentID should have 1 file, ${component.length} files found: ${component.mkString(
        ", "
      )}."""
    )
    component(0)
  ...
  override def version(): String =
    StaticUtils.getProperty(loader, "compiler.properties", "version.number") getOrElse "unknown"
}
...
private class StaticAppProvider(appConfig: StaticAppConfiguration) extends xsbti.AppProvider {
  ...
  override def components(): xsbti.ComponentProvider =
    new StaticComponentProvider(scalaProvider.launcher.bootDirectory)
  ...
  override def mainClass(): Class[xsbti.AppMain] =
    loader.loadClass(id.mainClass).asInstanceOf[Class[xsbti.AppMain]]
