...
implicit lazy val TestItemDetailFormat: JsonFormat[sbt.internal.protocol.testing.TestItemDetail] = new JsonFormat[sbt.internal.protocol.testing.TestItemDetail] {
  override def read[J](jsOpt: Option[J], unbuilder: Unbuilder[J]): sbt.internal.protocol.testing.TestItemDetail = {
    jsOpt match {
      case Some(js) =>
      ...
      unbuilder.endObject()
      sbt.internal.protocol.testing.TestItemDetail(fullyQualifiedName, status, duration)
      case None =>
      deserializationError("Expected JsObject but found None")
    }
  }
  override def write[J](obj: sbt.internal.protocol.testing.TestItemDetail, builder: Builder[J]): Unit = {
    builder.beginObject()
