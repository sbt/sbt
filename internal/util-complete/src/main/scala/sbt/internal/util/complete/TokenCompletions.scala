...
object TokenCompletions:
  private[sbt] abstract class Delegating extends TokenCompletions:
    outer =>
    ...
    final def hideWhen(hide: Int => Boolean): TokenCompletions = new Delegating:
      def completions(seen: String, level: Int, delegate: Completions): Completions =
        if hide(level) then Completions.nil else outer.completions(seen, level, delegate)

  private[sbt] abstract class Fixed extends TokenCompletions:
    outer =>
    ...
    final def hideWhen(hide: Int => Boolean): TokenCompletions = new Fixed:
      def completions(seen: String, level: Int) =
        if hide(level) then Completions.nil else outer.completions(seen, level)
