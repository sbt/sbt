...
import sbt.internal.scripted.{ TestException, FileCommands, TestScriptParser, ScriptRunner, CommentHandler }

object ScriptedRunnerImpl {
  def run(resourceBaseDirectory: File, bufferLog: Boolean, tests: Array[String]): Unit = {
    val runner = new ScriptedTests(resourceBaseDirectory, bufferLog)
...
final class ScriptedTests(resourceBaseDirectory: File, bufferLog: Boolean) {
