...
object Logic:
  ...
  private def checkOverlap(
    ...
    if initialOverlap.nonEmpty then Some(new InitialOverlap(initialOverlap)) else None
  ...
  private def checkContradictions(
    ...
    if contradictions.nonEmpty then Some(new InitialContradictions(contradictions)) else None
  ...
  @tailrec private def reduce0(
    ...
    applyAll(clauses, factsToProcess) match
      ...
      case Some(applied) =>
        ...
        if unprovenClauses.isEmpty then newState // no remaining clauses, done.
        else
          val unproven = Clauses(unprovenClauses)
          val nextFacts: Set[Literal] =
            if newlyProven.nonEmpty then newlyProven.toSet[Literal]
  ...
  private def inferFailure(clauses: Clauses): Set[Literal] =
    ...
    if newFacts.nonEmpty then newFacts
    else
      ...
      if newlyFalse.nonEmpty then newlyFalse
  ...
  def applyAll(cs: Clauses, facts: Set[Literal]): Option[Clauses] =
    val newClauses =
      ...
      else cs.clauses.map(c => applyAll(c, facts)).flatMap(_.toList)
    if newClauses.isEmpty then None else Some(Clauses(newClauses))

  def applyAll(c: Clause, facts: Set[Literal]): Option[Clause] =
    val atoms = facts.map(_.atom)
    val newHead = c.head -- atoms // 3.
    if newHead.isEmpty then // 4. empty head
      None
    else substitute(c.body, facts).map(f => Clause(f, newHead)) // 1, 2
  ...
  @tailrec def substitute(formula: Formula, facts: Set[Literal]): Option[Formula] = formula match
    case And(lits) =>
      ...
      if lits.exists(negated(facts)) then // 2.
        None
      else
        val newLits = lits -- facts
        val newF =
          if newLits.isEmpty then (True: Formula)
