...
private[sbt] final class ProgressState(
  ...
  private def appendLine(line: String) = while !lineBuffer.offer(line) do lineBuffer.poll
  private[util] def clearBytes(): Unit =
    val pad = padding.get
    if currentLineBytes.get.isEmpty && pad > 0 then padding.decrementAndGet()
    currentLineBytes.set(new ArrayBuffer[Byte])
  ...
  private[util] def addBytes(terminal: Terminal, bytes: Seq[Byte]): Unit =
    ...
    val prevLineCount = if padding > 0 then terminal.lineCount(new String(previous.toArray)) else 0
    previous ++= bytes
    if padding > 0 then
    ...
    if lines.contains(System.lineSeparator) then
      currentLineBytes.set(new ArrayBuffer[Byte])
      if !lines.endsWith(System.lineSeparator) then
      ...
      else if lines.contains(System.lineSeparator) then
        lines.split(System.lineSeparator).foreach(appendLine)

  private[util] def getPrompt(terminal: Terminal): Array[Byte] =
    if terminal.prompt.isInstanceOf[Prompt.AskUser] then
      val prefix = if terminal.isAnsiSupported then s"$DeleteLine$CursorLeft1000" else ""
  ...
  private[util] def write(
    ...
    if hasProgress then
      ...
      if endsWithNewLine || bytes.containsSlice(lineSeparatorBytes) then
        ...
        def appendLine(l: String, appendNewline: Boolean): Unit =
          ...
          if !l.getBytes("UTF-8").endsWith("\r".getBytes) then toWrite ++= clearScreenBytes
          if appendNewline then toWrite ++= lineSeparatorBytes
      ...
      if endsWithNewLine then
        if progressLines.get.nonEmpty then
  ...
  private[util] def printProgress(terminal: Terminal, lastLine: String): String =
    ...
    if previousLines.nonEmpty then
      ...
      val resetCursorUp = cursorUp(totalSize + (if offset then 1 else 0))
      val resetCursor = resetCursorUp + CursorLeft1000 + lastLine
      start + blank + lines + resetCursor
    else ClearScreenAfterCursor
end ProgressState
    ...
    if terminal.isSupershellEnabled then
      ...
      if !pe.skipIfActive.getOrElse(false) || (!isRunning && !isBatch) then
        terminal.withPrintStream { ps =>
          ...
          val lastLine =
            if isAskUser then terminal.prompt.render() else terminal.getLastLine.getOrElse("")
      ...
      else if state.progressLines.get.nonEmpty then
