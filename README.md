...
3. The initial branch is the development branch 0.11, which contains the latest code for the 0.11.x series.  To build a specific release or commit, switch to the associated tag.  The tag for the latest stable release is v0.11.2:

		$ git checkout v0.11.2

	Note that sbt is always built with the previous stable release.  For example, the 0.11 branch is built with 0.11.2, the v0.11.2 tag is built with 0.11.1, and the v0.10.1 tag is built with 0.10.0.
...
5. To use this locally built version of sbt, copy your stable ~/bin/sbt script to ~/bin/xsbt and change it to use the launcher jar in `<xsbt>/target/`.  For the v0.11.2 tag, the full location is:
	...
	If using the 0.11 development branch, the launcher is at:
		...
		<xsbt>/target/sbt-launch-0.11.3-SNAPSHOT.jar
...
If a project has `project/build.properties` defined, either delete the file or change `sbt.version` to `0.11.3-SNAPSHOT`.
