...
trait CompletionService:
   ...
   * sbt.Completed.
   */
  def submit(node: Task[?], work: () => Completed): Unit
...
object CompletionService {
  ...
  def apply(completion: JCompletionService[Completed]): CompletionService =
    new CompletionService {
      def submit(node: Task[?], work: () => Completed) = {
  ...
  )(setup: Task[?] => Unit, cleanup: Task[?] => Unit): CompletionService =
  ...
  def wrap(
      service: CompletionService
  )(w: (Task[?], () => Completed) => (() => Completed)): CompletionService =
    new CompletionService {
      def submit(node: Task[?], work: () => Completed) = service.submit(node, w(node, work))
