...
private[sbt] final class Execute(
  ...
  def processAll()(using strategy: CompletionService): Unit =
    @tailrec def next(): Unit =
      pre {
        ...
        if !state.values.exists(_ == Running) then
          snapshotCycleCheck()
          assert(
            false,
      ...
      try strategy.take().process()
      catch
        case e: ExecutionException =>
          e.getCause match
            case oom: OutOfMemoryError => throw oom
            case _                     => throw e
      if reverse.nonEmpty then next()
    next()
  ...
  def call[A](node: TaskId[A], target: TaskId[A])(using strategy: CompletionService): Unit =
    if config.checkCycles then cycleCheck(node, target)
    pre {
      assert(running(node))
      readyInv(node)
    }
    ...
    post {
      if done(target) then assert(done(node))
      else
        assert(calling(node))
  ...
  def addChecked[A](node: TaskId[A])(using strategy: CompletionService): Unit =
    if !added(node) then addNew(node)
  ...
  def addNew(node: TaskId[?])(using strategy: CompletionService): Unit =
    ...
    if active.isEmpty then ready(node)
    else
      forward(node) = active
      for a <- active do
        addChecked(a)
    ...
    post {
      ...
      if running(node) then runningInv(node)
      if pending(node) then pendingInv(node)
    }
  end addNew
  ...
  def work[A](node: TaskId[A], f: => Either[TaskId[A], A])(using
    ...
    val rawResult = wideConvert(f).left.map {
      case i: Incomplete => if config.overwriteNode(i) then i.copy(node = Some(node)) else i
  ...
  def snapshotCycleCheck(): Unit =
    callers.toSeq foreach { (called, callers) =>
      for caller <- callers do cycleCheck(caller, called)
    }

  def cycleCheck(node: TaskId[?], target: TaskId[?]): Unit =
    if node eq target then cyclic(node, target, "Cannot call self")
  ...
  def pre(f: => Unit) = if checkPreAndPostConditions then f
  def post(f: => Unit) = if checkPreAndPostConditions then f
end Execute
