...
private[sbt] final class SimpleCommand(val name: String,
...
private[sbt] final class ArbitraryCommand(val parser: State => Parser[() => State],
                                          val help: State => Help,
                                          val tags: AttributeMap)
    extends Command {
...
object Command {
  ...
  def apply[T](name: String,
               help: Help = Help.empty)(parser: State => Parser[T])(effect: (State, T) => State): Command =
