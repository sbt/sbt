...
object BasicCommands {
  ...
  private[this] def levelParser: Parser[String] =
    token(Level.Debug.toString) | token(Level.Info.toString) | token(Level.Warn.toString) | token(
      Level.Error.toString
    )
  private[this] def earlyParser: State => Parser[String] =
    (s: State) =>
  ...
  def append = Command(AppendCommand, Help.more(AppendCommand, AppendLastDetailed))(otherCommandParser) {
    (s, arg) =>
      s.copy(remainingCommands = s.remainingCommands :+ arg)
  }

  def setOnFailure = Command(OnFailure, Help.more(OnFailure, OnFailureDetailed))(otherCommandParser) {
    (s, arg) =>
  ...
  private[sbt] def compatCommands = Seq(
    ...
    Command.arb(s => token(Compat.OnFailure, hide = const(true)).flatMap(x => otherCommandParser(s))) {
      (s, arg) =>
    ...
    case (state, (cp, args)) =>
      ...
      state.log.info(
        "Applying State transformations " + args
          .mkString(", ") + (if (cp.isEmpty) "" else " from " + cp.mkString(File.pathSeparator))
      )
  ...
  private[this] def className: Parser[String] = {
    ...
    val compl = TokenCompletions.fixed(
      (seen, level) => if (seen.startsWith("-")) Completions.nil else single("<class name>")
    )
    token(base, compl)
  }
  ...
  def shell = Command.command(Shell, Help.more(Shell, ShellDetailed)) { s =>
    ...
    line match {
      case Some(line) =>
        val newState = s
          .copy(onFailure = Some(Shell), remainingCommands = line +: Shell +: s.remainingCommands)
          .setInteractive(true)
    ...
    arg match {
      case Left(portAndSuccess) =>
        ...
        readMessage(port, previousSuccess) match {
          case Some(message) =>
            (message :: (ReadCommand + " " + port) :: s).copy(onFailure = Some(ReadCommand + " " + (-port)))
          case None =>
  ...
  def isNamed(name: String, alias: Option[(String, String)]): Boolean = alias match {
  ...
  def newAlias(name: String, value: String): Command =
    Command
      .make(name, (name, "'" + value + "'"), "Alias of '" + value + "'")(aliasBody(name, value))
