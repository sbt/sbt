...
object State {
  ...
  implicit def stateOps(s: State): StateOps = new StateOps {
    ...
    def reboot(full: Boolean) = {
      runExitHooks(); throw new xsbti.FullReload(s.remainingCommands.toArray, full)
    }
    ...
    private[this] def newClassLoaderCache =
      new classpath.ClassLoaderCache(s.configuration.provider.scalaProvider.launcher.topLoader)
  }
