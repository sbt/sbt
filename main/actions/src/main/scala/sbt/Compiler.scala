...
object Compiler {
  ...
  final case class Options(classpath: Seq[File],
                           sources: Seq[File],
                           ...
                           order: CompileOrder)
  final case class IncSetup(analysisMap: File => Option[Analysis],
  ...
  def inputs(classpath: Seq[File],
             sources: Seq[File],
             classesDirectory: File,
             options: Seq[String],
             javacOptions: Seq[String],
             maxErrors: Int,
             sourcePositionMappers: Seq[Position => Option[Position]],
  ...
  def compilers(instance: ScalaInstance, cpOptions: ClasspathOptions)(implicit app: AppConfiguration,
                                                                      log: Logger): Compilers =
    compilers(instance, cpOptions, None)

  @deprecated(
    "Use `compilers(ScalaInstance, ClasspathOptions, Option[File], IvyConfiguration, ModuleID)`.",
    "0.13.10"
  )
  def compilers(instance: ScalaInstance,
                cpOptions: ClasspathOptions,
                javaHome: Option[File])(implicit app: AppConfiguration, log: Logger): Compilers = {
    val javac =
    ...
    case class CheaterJavaTool(newJavac: IncrementalCompilerJavaTools, delegate: JavaTool)
        extends JavaTool
        with JavaToolWithNewInterface {
      def compile(contract: JavacContract,
                  sources: Seq[File],
                  classpath: Seq[File],
                  outputDirectory: File,
                  options: Seq[String])(implicit log: Logger): Unit =
  ...
  def compilers(instance: ScalaInstance,
                ...
                sourcesModule: ModuleID)(implicit app: AppConfiguration, log: Logger): Compilers = {
    val javac =
      AggressiveCompile.directOrFork(instance, cpOptions, javaHome)
    val javac2 =
      JavaTools.directOrFork(instance, cpOptions, javaHome)
  ...
  def compilers(instance: ScalaInstance, cpOptions: ClasspathOptions, javac: sbt.compiler.JavaCompiler.Fork)(
  ...
  def compilers(instance: ScalaInstance,
                cpOptions: ClasspathOptions,
                javac: JavaTool)(implicit app: AppConfiguration, log: Logger): Compilers = {
  ...
  def scalaCompiler(instance: ScalaInstance, cpOptions: ClasspathOptions)(implicit app: AppConfiguration,
                                                                          log: Logger): AnalyzingCompiler = {
    val launcher = app.provider.scalaProvider.launcher
    val componentManager =
      new ComponentManager(launcher.globalLock, app.provider.components, Option(launcher.ivyHome), log)
  ...
  def compile(in: InputsWithPrevious, log: Logger, reporter: xsbti.Reporter): CompileResult = {
    ...
    IC.incrementalCompile(
      ...
      options,
      javacOptions,
      in.previousAnalysis.analysis,
      in.previousAnalysis.setup,
      analysisMap,
      definesClass,
      reporter,
      order,
