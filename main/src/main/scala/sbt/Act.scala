...
object Act {
  ...
  def makeScopedKey(proj: Option[ResolvedReference],
    ...
    ScopedKey(
      Scope(toAxis(proj, Global), toAxis(conf map ConfigKey.apply, Global), toAxis(task, Global), extra),
      key
    )

  def select(allKeys: Seq[Parser[ParsedKey]],
             data: Settings[Scope])(implicit show: Show[ScopedKey[_]]): Parser[ParsedKey] =
  ...
  def selectFromValid(ss: Seq[ParsedKey],
                      default: Parser[ParsedKey])(implicit show: Show[ScopedKey[_]]): Parser[ParsedKey] =
  ...
  def configs(explicit: ParsedAxis[String],
    ...
    explicit match {
      case Omitted =>
        None +: defaultConfigurations(proj, index, defaultConfigs).flatMap(nonEmptyConfig(index, proj))
      case ParsedGlobal       => None :: Nil
      case pv: ParsedValue[x] => Some(pv.value) :: Nil
    }
  def defaultConfigurations(proj: Option[ResolvedReference],
                            index: KeyIndex,
