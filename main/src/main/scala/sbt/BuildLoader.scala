  ...
  def apply(info: S): Option[T] =
    (baseLoader(info), applyNonRoots(info)) match {
      ...
      case (None, xs @ (_, nr) :: ignored) =>
        if (ignored.nonEmpty)
          warn("Using first of multiple matching non-root build resolvers for " + getURI(info), log(info), xs)
        Some(nr)
      case (Some(b), xs) =>
        if (xs.nonEmpty)
...
object BuildLoader {
  ...
  final class ResolveInfo(val uri: URI,
                          ...
                          val state: State)
      extends Info
  final class BuildInfo(val uri: URI, val base: File, val config: LoadBuildConfiguration, val state: State)
      extends Info
