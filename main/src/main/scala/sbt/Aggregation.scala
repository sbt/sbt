...
final object Aggregation {
  ...
  def defaultShow(state: State, showTasks: Boolean): ShowConfig =
    ShowConfig(settingValues = true, taskValues = showTasks, s => state.log.info(s), success = true)
  ...
  def applyTasks[T](s: State, structure: BuildStructure, ps: Values[Parser[Task[T]]], show: ShowConfig)(
  ...
  def runTasksWithResult[T](
      ...
      extra: DummyTaskMap,
      show: ShowConfig
  )(implicit display: Show[ScopedKey[_]]): (State, Result[Seq[KeyValue[T]]]) = {
  ...
  def projectAggregates[Proj](proj: Option[Reference],
    ...
    resRef.toList.flatMap(ref => if (reverse) extra.aggregates.reverse(ref) else extra.aggregates.forward(ref))
  }
