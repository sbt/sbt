...
private[sbt] final class ResolutionProgress:
  ...
  def onInit(): Unit =
    ...
    if inFlight.getAndIncrement() == 0 then burstStartNanos.set(now)
    ()
  ...
  def onFile(url: String): Unit =
    if !ResolutionProgress.isChecksumLike(url) then files.add(url)
    ()
  ...
  def onProgress(url: String, downloaded: Long): Unit =
    ...
    seen.compute(
      ...
      (_: String, prev: java.lang.Long) =>
        val p: Long = if prev == null then 0L else prev.longValue
        if downloaded > p then bytes.addAndGet(downloaded - p)
  ...
  def snapshot(): Option[(String, Long)] =
    if inFlight.get() <= 0 then None
    else
      val n = files.size()
      val mib = bytes.get().toDouble / (1024.0 * 1024.0)
      val label = if n == 1 then "file" else "files"
...
private[sbt] final class ResolutionProgressLogger(sink: ResolutionProgress) extends CacheLogger:
  ...
  override def downloadedArtifact(url: String, success: Boolean): Unit =
    if success then sink.onFile(url)
