...
object SessionSettings {
  ...
  def removeSettings(s: State, ranges: Seq[(Int, Int)]): State =
    withSettings(s) { session =>
      ...
      val newAppend =
        session.append.updated(current, removeRanges(session.append.getOrElse(current, Nil), ranges))
  ...
  def writeSettings(pref: ProjectRef,
    ...
    val writeTo: File =
      BuildPaths.configurationSources(project.base).headOption.getOrElse(new File(project.base, "build.sbt"))
