...
import Def.{ isDummy, ScopeLocal, ScopedKey, Setting }
import Keys.{
  appConfiguration,
  baseDirectory,
  configuration,
...
object Load {
  ...
  private[sbt] def useAutoImportInGlobal =
    sys.props.get("sbt.global.autoimport") map { _.toLowerCase == "true" } getOrElse false
  ...
  def reapply(newSettings: Seq[Setting[_]],
              structure: sbt.BuildStructure)(implicit display: Show[ScopedKey[_]]): sbt.BuildStructure = {
  ...
  def load(file: File, s: State, config: sbt.LoadBuildConfiguration): sbt.PartBuild =
    load(
      file,
      builtinLoader(s, config.copy(pluginManagement = config.pluginManagement.shift, extraBuilds = Nil)),
  ...
  ): (Map[URI, List[ProjectReference]], Map[URI, sbt.PartBuildUnit], BuildLoader) =
    bases match {
      case b :: bs =>
        if (builds contains b)
          ...
          val newLoader = addOverrides(
            loadedBuild.unit,
            addResolvers(loadedBuild.unit, builds.isEmpty, loaders.resetPluginDepth)
          )
  ...
  /**
    ...
    */
  private[this] def loadTransitive(newProjects: Seq[Project],
                                   buildBase: File,
  ...
  /**
    ...
    */
  private[this] def resolveProject(p: Project,
  ...
  /**
    ...
    */
  private[this] def discoverProjects(auto: AddSettings,
                                     projectBase: File,
                                     loadedPlugins: sbt.LoadedPlugins,
                                     eval: () => Eval,
    ...
    import AddSettings.{ AutoPlugins, BuildScalaFiles, DefaultSbtFiles, Plugins, SbtFiles, Sequence, User }
  ...
  def buildUtil(root: URI,
                ...
                keyIndex: KeyIndex,
                data: Settings[Scope]): BuildUtil[ResolvedProject] = BuildUtil(root, units, keyIndex, data)
...
final case class LoadBuildConfiguration(stagingDirectory: File,
  ...
  lazy val globalPluginNames =
    if (globalPluginClasspath.isEmpty) Nil else Load.getPluginNames(globalPluginClasspath, globalPluginLoader)
