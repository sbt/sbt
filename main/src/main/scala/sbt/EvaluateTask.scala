...
object EvaluateTask {
  ...
  private[sbt] def executeProgress(extracted: Extracted,
    ...
    import Types.const
    val maker: State => Keys.TaskProgress =
      getSetting(Keys.executeProgress, const(new Keys.TaskProgress(defaultProgress)), extracted, structure)
  ...
  def evaluateTask[T](structure: BuildStructure,
    ...
    apply(structure, taskKey, state, ref, EvaluateConfig(false, defaultRestrictions(maxWorkers), checkCycles))
      .map(_._2)
  ...
  def runTask[T](root: Task[T],
                 ...
                 config: EvaluateConfig)(implicit taskToNode: NodeView[Task]): (State, Result[T]) = {
  ...
  def runTask[T](root: Task[T],
    ...
    import ConcurrentRestrictions.{ completionService, tagged, tagsKey, Tag, TagMap }
