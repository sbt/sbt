...
object EvaluateTask:
  ...
  def taskTraceEvent: Option[ExecuteProgress] =
    if SysProp.traces then Some(sharedTraceEvent)
    else None
  ...
  def maxWorkers(extracted: Extracted, structure: BuildStructure): Int =
    if getSetting(Keys.parallelExecution, true, extracted, structure) then SystemProcessors
    else 1
  ...
  def logIncResult(result: Result[?], state: State, streams: Streams) = result match
    case Result.Inc(i) => logIncomplete(i, state, streams);
    case _             => ()
  ...
  def logIncomplete(result: Incomplete, state: State, streams: Streams): Unit =
    ...
    do
      ...
      ExceptionCategory(ex) match
        case AlreadyHandled => ()
        case m: MessageOnly => if msg.isEmpty then log.error(m.message)
        case f: Full        => log.trace(f.exception)

    for (key, msg, ex) <- keyed if msg.isDefined || ex.isDefined do
  ...
  private def contextDisplay(state: State, highlight: Boolean) =
    Project.showContextKey(state, if highlight then Some(RED) else None)
  ...
  def runTask[T](
    ...
    def shutdownImpl(force: Boolean): Unit =
      ...
      if config.forceGarbageCollection then
    ...
    def run() =
      ...
      val (newState, result) =
        ...
        catch case inc: Incomplete => (state, Result.Inc(inc))
        finally shutdown()
  ...
  private def storeValuesForPrevious(
    ...
    for referenced <- (Global / Previous.references).get(Project.structure(state).data) do
  ...
  def convertCyclic(c: Execute#CyclicException): String =
    (c.caller, c.target) match
      case (caller: Task[?], target: Task[?]) =>
        c.toString + (if caller eq target then "(task: " + name(caller) + ")"
  ...
  def processResult2[T](result: Result[T], show: Boolean = false): T =
    onResult(result) { v =>
      if show then println("Result: " + v); v
    }
  ...
  val injectStreams: ScopedKey[?] => Seq[Setting[?]] = scoped =>
    if scoped.key == streams.key then
