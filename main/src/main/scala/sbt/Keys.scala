...
object Keys {
  ...
  private[sbt] val taskTerminal = AttributeKey[sbt.internal.util.Terminal]("task-terminal", Invisible)
  ...
  val runInBackground = settingKey[Boolean]("Toggles whether commands that can be run in the background block the sbt main loop").withRank(DSetting)
