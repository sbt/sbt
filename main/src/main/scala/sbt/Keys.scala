...
object Keys {
  ...
  val scalaInstance = taskKey[ScalaInstance]("Defines the Scala instance to use for compilation").withRank(DTask)
  val runtimeScalaInstance = taskKey[ScalaInstance]("Defines the Scala instance used at runtime (also for tests).").withRank(DTask)
  ...
  val scalaVersion = settingKey[String]("The version of Scala compiler used for building this project.").withRank(APlusSetting)
  val scalaOutputVersion = settingKey[String]("The version of Scala standard library used for running this project and declared as its transitive dependency.").withRank(APlusSetting)
