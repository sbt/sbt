...
object Resolvers:
  ...
  val local: Resolver = (info: ResolveInfo) =>
    ...
    if from.isDirectory then
      Some { () =>
        if from.canWrite then from else creates(to) { IO.copyDirectory(from, to) }
      }
    else None
  ...
  private def collectTransitiveRemoteBuildURIs(
    ...
    def go(
      ...
      queue match
        case Nil         => result
        case ref :: rest =>
          if visited.contains(ref) then go(rest, visited, result)
          else
            ...
            val newResult = allDeps.foldLeft(result) { (acc, dep) =>
              if dep.build != startRef.build then acc + dep.build else acc
            }
            go(allDeps.toList ::: rest, visited + ref, newResult)
    go(List(startRef), Set.empty, Set.empty)
  end collectTransitiveRemoteBuildURIs
  ...
  private def updateGit(localCopy: File, uri: URI, log: Logger): Boolean =
    try
      ...
      if status.nonEmpty then
        ...
        val ref = if fromURI(uri).hasFragment then
  ...
  private def updateSubversion(localCopy: File, uri: URI, log: Logger): Boolean =
    try
      ...
      else run(Some(localCopy), Some(log), "svn", "update", "-q")
      markUpdated(localCopy)
