...
object Watch:
  ...
  extension (event: Event) private[sbt] def toEpochString: String = event.occurredAt.toEpochString
  ...
  object ContinueWatch:
    ...
    given ordering: Ordering[ContinueWatch] = (left: ContinueWatch, right: ContinueWatch) =>
      left match
        case ShowOptions => if right == ShowOptions then 0 else -1
        case Ignore      => if right == Ignore then 0 else 1
        case Trigger     => if right == Trigger then 0 else if right == ShowOptions then 1 else -1
      ...
      left match
        ...
        case CancelWatch =>
          right match
            ...
            case Reload      => -1
            case _           => 1
        case Reload => if right == Reload then 0 else 1
  end CancelWatch
    ...
    @tailrec def impl(count: Int): Watch.Action =
      ...
      safeNextAction(count, onStart) match
        case Ignore =>
          next(count) match
            case Trigger =>
              if recursive then impl(count + 1)
              else
                task(count)
                Watch.Trigger
            case action => action
  ...
  private[sbt] def aggregate(events: Seq[(Action, Event)]): Option[(Action, Event)] =
    if events.isEmpty then None else Some(events.minBy(_._1))

  extension (s: String) private def toExec: Exec = Exec(s, None)
  ...
  private def distinctOptions(options: Seq[InputOption]): Seq[InputOption] =
    ...
    (options match
      ...
      if distinctOpts.add(o.input) then opts += o
    }
    opts.toSeq.reverse
