...
object ProjectNavigation {
  ...
  def getChannelCursor(s: State): Option[ProjectRef] =
    for {
      ...
      ref <- cursors.get(source.channelName)
    } yield ref
  ...
  def setChannelCursor(s: State, ref: ProjectRef): State =
    s.source match {
      case Some(source) =>
        ...
        s.put(channelProjectCursors, cursors.updated(source.channelName, ref))
      case None => s
    }

  def effectiveCurrentRef(s: State): ProjectRef =
    getChannelCursor(s).getOrElse(Project.extract(s).currentRef)
}
...
final class ProjectNavigation(s: State) {
  ...
  def setProject(nuri: URI, nid: String): State = {
    val newRef = ProjectRef(nuri, nid)
    s.source match {
      case Some(_) =>
        val s1 = ProjectNavigation.setChannelCursor(s, newRef)
        Project.updateCurrent(s1)
      case None =>
