...
final class ProjectNavigation(s: State):
  ...
  def setProject(nuri: URI, nid: String): State =
    val neval = if effectiveCurrentRef.build == nuri then session.currentEval else mkEval(nuri)
  ...
  def selectProject(uri: URI, to: String): State =
    if structure.units(uri).defined.contains(to) then setProject(uri, to)
    else
      fail(
  ...
  def changeBuild(newBuild: URI): State =
    if structure.units contains newBuild then setProject(newBuild, getRoot(newBuild))
    else fail("Invalid build unit '" + newBuild + "' (type 'projects' to list available builds).")

  def fail(msg: String): State =
    s.log.error(msg); s.fail
