...
private[sbt] object InstallSbtn:
  ...
  private[sbt] def installSbtnImpl: Def.Initialize[InputTask[Unit]] = Def.inputTask {
    ...
    val sbtn = if Properties.isWin then "sbtn.exe" else "sbtn"
    try extractSbtn(term, version, tmp, baseDirectory.resolve("bin").resolve(sbtn))
    finally
      Files.deleteIfExists(tmp)
      ()
    val shell = if hasConsole then getShell(term) else "none"
    shell match
      case "none" =>
      case s      =>
  ...
  private[sbt] def extractSbtn(term: Terminal, version: String, sbtZip: Path, sbtn: Path): Unit =
    ...
    val bin =
      if Properties.isWin then "pc-win32.exe"
    ...
    val arch =
      if Properties.isWin then "x86_64"
      else if Properties.isLinux && isArmArchitecture then "aarch64"
      else "universal"
    ...
    try
      ...
      while entry != null do
        if entry.getName == sbtnName then
          foundBinary = true
          term.printStream.println(s"extracting $sbtZip!$sbtnName to $sbtn")
          transfer(zipInputStream, sbtn)
          sbtn.toFile.setExecutable(true)
          entry = null
        else entry = zipInputStream.getNextEntry
      if !foundBinary then throw new IllegalStateException(s"couldn't find $sbtnName in $sbtZip")
    finally
      fis.close()
  ...
  private def setupShell(
    ...
    if !newContents.contains(sourceCompletions) then
      ...
      term.inputStream.read() match
        ...
        case c   =>
          ...
          else IO.write(configFile, s"$newContents\n$sourceCompletions")
