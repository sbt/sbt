...
object EvaluateConfigurations {
    ...
    val (importDefs, definitions) =
      if (parsed.definitions.isEmpty) (Nil, DefinedSbtValues.empty)
        ...
        val projs = (loader: ClassLoader) =>
          definitions.values(loader).map(p => resolveBase(file.getParentFile, p.asInstanceOf[Project]))
  ...
  /**
    ...
    */
  private[sbt] def splitExpressions(file: File,
  ...
  def groupedLines(lines: Seq[(String, Int)],
...
object Index {
  def taskToKeyMap(data: Settings[Scope]): Map[Task[_], ScopedKey[Task[_]]] = {
    ...
    val pairs = for (scope <- data.scopes; AttributeEntry(key, value: Task[_]) <- data.data(scope).entries)
      yield
