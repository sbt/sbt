...
object LogManager
{
	...
	def withLoggers(screen: (ScopedKey[_], State) => AbstractLogger = (sk: ScopedKey[_], s: State) => defaultScreen(s.globalLogging.console),
		backed: PrintWriter => AbstractLogger = defaultBacked(),
		extra: ScopedKey[_] => Seq[AbstractLogger] = (_: ScopedKey[_]) => Nil): LogManager = new LogManager {
