...
sealed trait Project extends ProjectDefinition[ProjectReference] {
  ...
  def setSbtFiles(files: File*): Project =
    copy(auto = AddSettings.append(AddSettings.clearSbtFiles(auto), AddSettings.sbtFiles(files: _*)))
...
object Project extends ProjectExtra {
  ...
  private abstract class ProjectDef[PR <: ProjectReference](val id: String,
                                                            val base: File,
                                                            aggregate0: => Seq[PR],
  ...
  def makeSettings(settings: Seq[Def.Setting[_]],
                   ...
                   scopeLocal: ScopedKey[_] => Seq[Def.Setting[_]])(implicit display: Show[ScopedKey[_]]) =
  ...
  def relation(structure: BuildStructure,
               actual: Boolean)(implicit display: Show[ScopedKey[_]]): Relation[ScopedKey[_], ScopedKey[_]] =
  ...
  def evaluateTask[T](taskKey: ScopedKey[Task[T]],
                      state: State,
                      checkCycles: Boolean = false,
                      maxWorkers: Int = EvaluateTask.SystemProcessors): Option[Result[T]] =
    runTask(taskKey, state, EvaluateConfig(true, EvaluateTask.defaultRestrictions(maxWorkers), checkCycles))
      .map(_._2)
  ...
  final class RichTaskSessionVar[S](i: Def.Initialize[Task[S]]) {
    ...
    def storeAs(key: TaskKey[S])(implicit f: sbinary.Format[S]): Def.Initialize[Task[S]] =
    ...
    def keepAs(key: TaskKey[S]): Def.Initialize[Task[S]] =
      (i, Keys.resolvedScoped)(
        (t, scoped) => tx(t, (state, value) => set(resolveContext(key, scoped.scope, state), state, value))
      )
  }
...
trait ProjectExtra {
  ...
  implicit def classpathDependency[T <% ProjectReference](p: T): ClasspathDependency =
    new ClasspathDependency(p, None)
