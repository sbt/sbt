...
object ParseKey extends Properties("Key parser test") {
  ...
  property("An explicitly specified axis is always parsed to that explicit value") =
    forAllNoShrink(structureDefinedKey) { (skm: StructureKeyMask) =>
      ...
      val hasZero = key.scope.config == Global || key.scope.config == Global
      val mask =
        mask0 match {
          case _ if hasZero => mask0.copy(project = true, config = true)
          case _            => mask0
        }
      ...
      val string = displayMasked(key, mask, hasZero)
  ...
  def parseExpected(structure: Structure, s: String, expected: ScopedKey[_], mask: ScopeMask): Prop =
    ("Expected: " + displayPedantic(expected)) |:
      ("Mask: " + mask) |:
      parse(structure, s) {
        case Left(err) => false
        case Right(sk) =>
          (s"Project.equal($sk, expected = $expected, $mask)") |:
