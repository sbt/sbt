...
object GenericPublisherTest extends Properties:
  ...
  /**
   * Publishes `content` into an Ivy-layout file repository twice over, with overwrite disabled
   * on every publish, and hands back the log so callers can assert on the warnings.
   */
  ...
  def fileUrlRepositoryPublishes: Result =
    withTempDir: dir =>
      val repo = dir / "repo"
      val pattern =
        s"${repo.toURI.toString.stripSuffix("/")}/[organisation]/[module]/[revision]/[type]s/[artifact](-[classifier]).[ext]"
      val patterns = Patterns()
        .withIvyPatterns(Vector(pattern))
        .withArtifactPatterns(Vector(pattern))
        .withIsMavenCompatible(false)
      val p = publisher(release, Seq(URLRepository("local", patterns)))
