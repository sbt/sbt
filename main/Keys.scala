...
//	val onComplete = SettingKey[RMap[Task,Result] => RMap[Task,Result]]("on-complete", "Transformation to apply to the final task result map.  This may also be used to register hooks to run when task evaluation completes.")
	...
	val globalPluginsDirectory = AttributeKey[File]("global-plugins-directory", "The base directory for global sbt plugins.")
	val globalSettingsDirectory = AttributeKey[File]("global-settings-directory", "The base directory for global sbt settings.")
	val stagingDirectory = AttributeKey[File]("staging-directory", "The directory for staging remote projects.")
