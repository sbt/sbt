...
object Inputs:
  ...
  def coursierConfigurationsMap(
    ...
    def allExtends(c: Configuration) =
      ...
      def helper(current: Set[Configuration]): Set[Configuration] =
        ...
        if (newSet -- current).nonEmpty then helper(newSet)
        else newSet
    ...
    def helper(
      ...
      toAdd match
        ...
        case config :: rest =>
          ...
          if missingExtends.isEmpty then (config, extends0) #:: helper(done + config, rest)
          else helper(done, missingExtends.toList ::: toAdd)
  ...
  def ivyGraphs(configurations: Map[Configuration, Seq[Configuration]]): Seq[Set[Configuration]] =
    ...
    for k <- configurations.keys do
      val s = sets(k)

      var foundNew = true
      while foundNew do
        foundNew = false
        for other <- s.set.toVector do
  ...
  def exclusionsSeq(
    ...
    val res = excludeDeps
      .flatMap { rule =>
        if rule.artifact != "*" || rule.configurations.nonEmpty then
    ...
    if anyNonSupportedExclusionRule then
