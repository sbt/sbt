...
final class DeliverConfiguration(val deliverIvyPattern: String,
                                 val status: String,
                                 val configurations: Option[Seq[Configuration]],
                                 val logging: UpdateLogging.Value)
final class PublishConfiguration(val ivyFile: Option[File],
                                 val resolverName: String,
                                 val artifacts: Map[Artifact, File],
                                 val checksums: Seq[String],
                                 val logging: UpdateLogging.Value,
                                 val overwrite: Boolean) {
  def this(ivyFile: Option[File],
           resolverName: String,
...
final class UpdateConfiguration(val retrieve: Option[RetrieveConfiguration],
                                val missingOk: Boolean,
                                val logging: UpdateLogging.Value) {
  private[sbt] def copy(retrieve: Option[RetrieveConfiguration] = this.retrieve,
...
final class RetrieveConfiguration(val retrieveDirectory: File,
                                  val outputPattern: String,
                                  val sync: Boolean,
                                  val configurationsToRetrieve: Option[Set[Configuration]]) {
  def this(retrieveDirectory: File, outputPattern: String) =
    this(retrieveDirectory, outputPattern, false, None)
  def this(retrieveDirectory: File, outputPattern: String, sync: Boolean) =
    this(retrieveDirectory, outputPattern, sync, None)
}
final case class MakePomConfiguration(file: File,
...
final case class GetClassifiersConfiguration(module: GetClassifiersModule,
                                             exclude: Map[ModuleID, Set[String]],
                                             configuration: UpdateConfiguration,
                                             ivyScala: Option[IvyScala])
final case class GetClassifiersModule(id: ModuleID,
                                      modules: Seq[ModuleID],
                                      configurations: Seq[Configuration],
                                      classifiers: Seq[String])

final class UnresolvedWarningConfiguration private[sbt] (val modulePositions: Map[ModuleID, SourcePosition])
...
object IvyActions {
  ...
  def mapArtifacts(module: ModuleDescriptor,
                   cross: Option[String => String],
                   artifacts: Map[Artifact, File]): Seq[(IArtifact, File)] = {
  ...
  /**
    ...
    * 'updateConfig' configures the actual resolution and retrieval process.
    */
  private[sbt] def updateEither(module: IvySbt#Module,
                                configuration: UpdateConfiguration,
      ...
      case (ivy, md, default)
        ...
        ivy.getResolveEngine match {
          case x: CachedResolutionResolveEngine =>
            ...
            x.customResolve(md, configuration.missingOk, logicalClock, resolveOptions, depDir getOrElse {
        ...
        err match {
          ...
          case _ =>
            val cachedDescriptor =
              ivy.getSettings.getResolutionCacheManager.getResolvedIvyFileInCache(md.getModuleRevisionId)
  ...
  def groupedConflicts[T](moduleFilter: ModuleFilter,
                          grouping: ModuleID => T)(report: UpdateReport): Map[T, Set[String]] =
  ...
  @deprecated("This is no longer public.", "0.13.6")
  def transitiveScratch(ivySbt: IvySbt,
                        label: String,
  ...
  private[sbt] def transitiveScratch(ivySbt: IvySbt,
                                     label: String,
                                     config: GetClassifiersConfiguration,
                                     uwconfig: UnresolvedWarningConfiguration,
                                     logicalClock: LogicalClock,
    ...
    val module = new ivySbt.Module(
      InlineConfigurationWithExcludes(base, ModuleInfo(base.name), deps).copy(ivyScala = ivyScala)
    )
  ...
  private[sbt] def updateClassifiers(ivySbt: IvySbt,
                                     config: GetClassifiersConfiguration,
                                     uwconfig: UnresolvedWarningConfiguration,
                                     logicalClock: LogicalClock,
                                     depDir: Option[File],
    ...
    val module = new ivySbt.Module(
      InlineConfigurationWithExcludes(base, ModuleInfo(base.name), deps)
        .copy(ivyScala = ivyScala, configurations = confs)
    )
  ...
  def classifiedArtifacts(classifiers: Seq[String],
                          exclude: Map[ModuleID, Set[String]])(m: ModuleID): Option[ModuleID] = {
    ...
    else Some(m.copy(isTransitive = false, explicitArtifacts = classifiedArtifacts(m.name, included)))
  }
  def addExcluded(report: UpdateReport,
                  classifiers: Seq[String],
                  exclude: Map[ModuleID, Set[String]]): UpdateReport =
  ...
  private[this] def restrictedCopy(m: ModuleID, confs: Boolean) =
    ModuleID(
      ...
      configurations = if (confs) m.configurations else None
    )
  ...
  private def retrieve(log: Logger,
                       ivy: Ivy,
                       report: UpdateReport,
                       config: RetrieveConfiguration): UpdateReport =
    retrieve(
      log,
      ivy,
      report,
  ...
  private def retrieve(log: Logger,
                       ...
                       pattern: String,
                       sync: Boolean,
                       configurationsToRetrieve: Option[Set[Configuration]]): UpdateReport = {
  ...
  private def performRetrieve(conf: String,
                              mid: ModuleID,
                              art: Artifact,
                              base: File,
                              pattern: String,
                              cached: File,
                              toCopy: collection.mutable.HashSet[(File, File)]): File = {
  ...
  import UpdateLogging.{ Default, DownloadOnly, Full, Quiet }
  import LogOptions.{ LOG_DEFAULT, LOG_DOWNLOAD_ONLY, LOG_QUIET }
  ...
  def publish(module: ModuleDescriptor,
...
final class ResolveException(val messages: Seq[String],
...
/**
  ...
  */
final class UnresolvedWarning private[sbt] (val resolveException: ResolveException,
...
object UnresolvedWarning {
  ...
  private[sbt] def apply(err: ResolveException,
                         failedPaths: Seq[Seq[(ModuleID, Option[SourcePosition])]]): UnresolvedWarning =
