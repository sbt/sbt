...
private[sbt] class UpdateUtil(ivyConfiguration: IvyConfiguration, log: Logger) {
  ...
  def getModule(moduleId: ModuleID,
                deps: Seq[ModuleID],
                uo: UpdateOptions = UpdateOptions(),
                ivyScala: Option[IvyScala]): ivySbt.Module = {
  ...
  def update(module: ivySbt.Module, retrieveDirectory: File)(predicate: File => Boolean): Option[Seq[File]] = {
    ...
    val retrieveConfiguration =
      new RetrieveConfiguration(retrieveDirectory, Resolver.defaultRetrievePattern, false)
    val updateConfiguration =
      new UpdateConfiguration(Some(retrieveConfiguration), true, UpdateLogging.DownloadOnly)
