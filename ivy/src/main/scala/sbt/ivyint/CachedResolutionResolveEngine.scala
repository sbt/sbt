...
import core.report.{ ConfigurationResolveReport, DownloadReport, ResolveReport }
...
private[sbt] class CachedResolutionResolveCache() {
  ...
  val conflictCache: concurrent.Map[(ModuleID, ModuleID), (Vector[ModuleID], Vector[ModuleID], String)] =
    concurrent.TrieMap()
  ...
  def buildArtificialModuleDescriptor(
    ...
    val md1 =
      new DefaultModuleDescriptor(createID(sbtOrgTemp, "temp-resolve-" + sha1, "1.0"), "release", null, false)
  ...
  def getOrElseUpdateMiniGraph(
      ...
      log: Logger
  )(f: => Either[ResolveException, UpdateReport]): Either[ResolveException, UpdateReport] = {
    import Path._
  ...
  def getOrElseUpdateConflict(cf0: ModuleID, cf1: ModuleID, conflicts: Vector[ModuleReport])(
    ...
    def reconstructReports(surviving: Vector[ModuleID],
                           evicted: Vector[ModuleID],
                           mgr: String): (Vector[ModuleReport], Vector[ModuleReport]) = {
      val moduleIdMap = Map(conflicts map { x =>
        x.module -> x
      }: _*)
      (surviving map moduleIdMap, evicted map moduleIdMap map {
        _.copy(evicted = true, evictedReason = Some(mgr.toString))
      })
    }
...
private[sbt] trait CachedResolutionResolveEngine extends ResolveEngine {
  ...
  /**
    ...
    */
  def customResolve(md0: ModuleDescriptor,
                    missingOk: Boolean,
                    logicalClock: LogicalClock,
                    options0: ResolveOptions,
                    depDir: File,
                    log: Logger): Either[ResolveException, UpdateReport] =
  ...
  def mergeResults(md0: ModuleDescriptor,
  ...
  def mergeReports(md0: ModuleDescriptor,
    ...
    val stats =
      new UpdateStats(resolveTime, (cachedReports map { _.stats.downloadTime }).sum, (cachedReports map {
        _.stats.downloadSize
      }).sum, false)
  ...
  def mergeConfigurationReports(rootModuleConf: String,
                                reports: Vector[ConfigurationReport],
                                os: Vector[IvyOverride],
                                log: Logger): ConfigurationReport = {
  ...
  /**
    ...
    */
  def mergeOrganizationArtifactReports(rootModuleConf: String,
                                       reports0: Vector[OrganizationArtifactReport],
                                       os: Vector[IvyOverride],
                                       log: Logger): Vector[OrganizationArtifactReport] = {
    ...
    @tailrec
    def sortModules(cs: Vector[(String, String)],
                    acc: Vector[(String, String)],
                    extra: Vector[(String, String)],
                    n: Int,
                    guard: Int): Vector[(String, String)] = {
  ...
  /**
    ...
    */
  def mergeModuleReports(rootModuleConf: String,
                         modules: Seq[ModuleReport],
                         os: Vector[IvyOverride],
                         log: Logger): (Vector[ModuleReport], Vector[ModuleReport]) = {
    if (modules.nonEmpty) {
      log.debug(
  ...
  /**
    ...
    */
  def transitivelyEvict(rootModuleConf: String,
                        pairs: List[(String, String)],
                        reports0: Map[(String, String), Vector[OrganizationArtifactReport]],
                        evicted0: Vector[ModuleReport],
                        log: Logger): Map[(String, String), Vector[OrganizationArtifactReport]] = {
  ...
  /**
    ...
    */
  def resolveConflict(rootModuleConf: String,
                      conflicts: Vector[ModuleReport],
                      os: Vector[IvyOverride],
                      log: Logger): (Vector[ModuleReport], Vector[ModuleReport]) = {
    import org.apache.ivy.plugins.conflict.{ LatestConflictManager, NoConflictManager, StrictConflictManager }
    val head = conflicts.head
    val organization = head.module.organization
    val name = head.module.name
    log.debug(
      s"::: resolving conflict in $rootModuleConf:$organization:$name " + (conflicts map { _.module })
        .mkString("(", ", ", ")")
    )
      ...
      (conflicts find { m =>
        ...
        case Some(m) =>
          ...
          (Vector(m), conflicts filterNot { _ == m } map {
            _.copy(evicted = true, evictedReason = Some("direct-force"))
          }, "direct-force")
        case None =>
          (conflicts find { m =>
            ...
            case Some(m) if !ignoreTransitiveForce =>
              ...
              (Vector(m), conflicts filterNot { _ == m } map {
                _.copy(evicted = true, evictedReason = Some("transitive-force"))
              }, "transitive-force")
            case _ =>
              ...
              Option(strategy.findLatest(infos.toArray, None.orNull)) match {
                case Some(ModuleReportArtifactInfo(m)) =>
                  (Vector(m), conflicts filterNot { _ == m } map {
                    _.copy(evicted = true, evictedReason = Some(lcm.toString))
                  }, lcm.toString)
      ...
      os find { ovr =>
        ...
        case Some(ovr) if Option(ovr.ddm.getVersion).isDefined =>
          ...
          conflicts find { mr =>
            ...
            case Some(m) =>
              (Vector(m), conflicts filterNot { _ == m } map {
                _.copy(evicted = true, evictedReason = Some("override"))
              }, "override")
            case None =>
              sys.error(
        ...
        case None =>
          getSettings.getConflictManager(IvyModuleId.newInstance(organization, name)) match {
            ...
            case _: StrictConflictManager =>
              sys.error((s"conflict was found in $rootModuleConf:$organization:$name " + (conflicts map {
                _.module
              }).mkString("(", ", ", ")")))
  ...
  def remapInternalProject(node: IvyNode,
                           ur: UpdateReport,
                           md0: ModuleDescriptor,
                           dd: DependencyDescriptor,
                           os: Vector[IvyOverride],
