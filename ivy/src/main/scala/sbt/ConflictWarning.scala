...
object ConflictWarning {
  ...
  def apply(label: String,
            filter: ModuleFilter,
            group: ModuleID => String,
            level: Level.Value,
            failOnConflict: Boolean): ConflictWarning =
  ...
  @deprecated(
    "Warning on evicted modules is no longer done, so this is the same as `default`.  Use a standard Ivy conflict manager.",
    "0.13.0"
  )
