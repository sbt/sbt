...
class IvyCache(val ivyHome: Option[File]) {
  ...
  def withDefaultCache[T](lock: Option[xsbti.GlobalLock],
                          log: Logger)(f: DefaultRepositoryCacheManager => T): T = {
  ...
  private def basicLocalIvy(lock: Option[xsbti.GlobalLock], log: Logger) = {
    ...
    val conf =
      new InlineIvyConfiguration(paths, Seq(local), Nil, Nil, false, lock, IvySbt.DefaultChecksums, None, log)
