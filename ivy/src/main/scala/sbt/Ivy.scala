...
import ivyint.{ CachedResolutionResolveCache, CachedResolutionResolveEngine, SbtDefaultDependencyDescriptor }
...
final class IvySbt(val configuration: IvyConfiguration) {
    ...
    val i = new Ivy() {
      ...
      override def bind(): Unit = {
        ...
        if (configuration.updateOptions.cachedResolution) {
          setResolveEngine(
            new ResolveEngine(getSettings, getEventManager, getSortEngine) with CachedResolutionResolveEngine {
  ...
  final class Module(rawModuleSettings: ModuleSettings) {
    ...
    private def newConfiguredModuleID(module: ModuleID,
    ...
    private def addScalaToolDependencies(dmd: DefaultModuleDescriptor,
...
private[sbt] object IvySbt {
  ...
  private[sbt] val cachedResolutionResolveCache: CachedResolutionResolveCache =
  ...
  /**
    ...
    */
  private def setResolvers(settings: IvySettings,
  ...
  def resolverChain(name: String,
                    resolvers: Seq[Resolver],
                    localOnly: Boolean,
                    settings: IvySettings,
                    log: Logger): DependencyResolver =
  ...
  def resolverChain(name: String,
    ...
    if (projectResolvers.isEmpty)
      ...
      val delegate =
        new ivyint.SbtChainResolver(name + "-delegate", mapResolvers(rest), settings, updateOptions, log)
  ...
  private def setModuleConfigurations(settings: IvySettings,
  ...
  private[this] def configureRepositoryCache(settings: IvySettings,
                                             localOnly: Boolean) //, artifactResolver: DependencyResolver)
  {
    val cacheDir = settings.getDefaultRepositoryCacheBasedir()
    val manager = new DefaultRepositoryCacheManager("default-cache", settings, cacheDir) {
      override def findModuleInCache(dd: DependencyDescriptor,
      ...
      override def saveResolvers(descriptor: ModuleDescriptor,
                                 metadataResolverName: String,
                                 artifactResolverName: String): Unit = ()
    }
  ...
  private def addMainArtifact(moduleID: DefaultModuleDescriptor): Unit = {
    val artifact =
      DefaultArtifact.newIvyArtifact(moduleID.getResolvedModuleRevisionId, moduleID.getPublicationDate)
    moduleID.setModuleArtifact(artifact)
    moduleID.check()
  }
  private def setConflictManager(moduleID: DefaultModuleDescriptor,
  ...
  private def substituteCross(m: ModuleSettings,
                              scalaFullVersion: String,
                              scalaBinaryVersion: String): ModuleSettings = {
    ...
    m match {
      ...
      case ic: InlineConfiguration =>
        ic.copy(
          module = sub(ic.module),
          dependencies = ic.dependencies map sub,
          overrides = ic.overrides map sub
        )
      case ic: InlineConfigurationWithExcludes =>
        ic.copy(
          module = sub(ic.module),
          dependencies = ic.dependencies map sub,
          overrides = ic.overrides map sub
  ...
  private def toIvyArtifact(moduleID: ModuleDescriptor,
                            a: Artifact,
                            allConfigurations: Iterable[String]): MDArtifact = {
  ...
  private def parseIvyXML(settings: IvySettings,
                          xml: scala.xml.NodeSeq,
                          moduleID: DefaultModuleDescriptor,
                          defaultConfiguration: String,
                          validate: Boolean): CustomXmlParser.CustomParser =
  ...
  def addDependencies(moduleID: DefaultModuleDescriptor,
                      dependencies: Seq[ModuleID],
                      parser: CustomXmlParser.CustomParser): Unit = {
  ...
  def convertDependency(moduleID: DefaultModuleDescriptor,
                        dependency: ModuleID,
                        parser: CustomXmlParser.CustomParser): DefaultDependencyDescriptor = {
    ...
    for (artifact <- dependency.explicitArtifacts) {
      import artifact.{ `type`, classifier, extension, name, url }
    ...
    for (excls <- dependency.exclusions) {
      for (conf <- dependencyDescriptor.getModuleConfigurations) {
        dependencyDescriptor.addExcludeRule(
          conf,
          IvyScala.excludeRule(excls.organization, excls.name, excls.configurations, excls.artifact)
        )
      }
    }
    dependencyDescriptor
  ...
  private[this] def copyConfigurations(artifact: Artifact,
                                       addConfiguration: String => Unit,
                                       allConfigurations: Iterable[String]): Unit = {
  ...
  def addExcludes(moduleID: DefaultModuleDescriptor,
                  excludes: Seq[SbtExclusionRule],
                  ivyScala: Option[IvyScala]): Unit =
    excludes foreach addExclude(moduleID, ivyScala)
  def addExclude(moduleID: DefaultModuleDescriptor,
                 ivyScala: Option[IvyScala])(exclude0: SbtExclusionRule): Unit = {
  ...
  def addOverrides(moduleID: DefaultModuleDescriptor,
                   overrides: Set[ModuleID],
                   matcher: PatternMatcher): Unit =
