...
import org.apache.ivy.util.{ ChecksumHelper, FileUtil }
...
private[sbt] object ConvertResolver {
    ...
    case (r, settings, log) =>
      r match {
        case repo: MavenRepository => {
          val pattern =
            Collections.singletonList(Resolver.resolvePattern(repo.root, Resolver.mavenStyleBasePattern))
  ...
  private def initializeSSHResolver(resolver: AbstractSshBasedResolver,
                                    repo: SshBasedRepository,
                                    settings: IvySettings): Unit = {
    resolver.setName(repo.name)
    resolver.setPassfile(null)
    initializePatterns(resolver, repo.patterns, settings)
    initializeConnection(resolver, repo.connection)
  }
  private def initializeConnection(resolver: AbstractSshBasedResolver,
                                   connection: RepositoryHelpers.SshConnection): Unit = {
    import resolver._
    import connection._
  ...
  private def initializePatterns(resolver: AbstractPatternsBasedResolver,
    ...
    resolver.setDescriptor(
      if (patterns.descriptorOptional) BasicResolver.DESCRIPTOR_OPTIONAL else BasicResolver.DESCRIPTOR_REQUIRED
    )
