...
class MakePom(val log: Logger) {
  ...
  def makeProperties(module: ModuleDescriptor, dependencies: Seq[DependencyDescriptor]): NodeSeq = {
    ...
    val allExtra =
      if (depExtra.isEmpty) extra else extra.updated(PomExtraDependencyAttributes.ExtraAttributesKey, depExtra)
  ...
  def makeProperties(extra: Map[String, String]): NodeSeq = {
    def _extraAttributes(k: String) =
      if (k == PomExtraDependencyAttributes.ExtraAttributesKey) xmlSpacePreserve else scala.xml.Null
    <properties> {
  ...
  def makeDependencies(dependencies: Seq[DependencyDescriptor],
                       includeTypes: Set[String],
                       excludes: Seq[ExcludeRule]): NodeSeq =
  ...
  def makeDependency(dependency: DependencyDescriptor,
                     includeTypes: Set[String],
                     excludes: Seq[ExcludeRule]): NodeSeq = {
  ...
  def makeDependencyElem(dependency: DependencyDescriptor,
                         artifact: DependencyArtifactDescriptor): Option[Elem] =
    makeDependencyElem(dependency, artifact, Nil)

  def makeDependencyElem(dependency: DependencyDescriptor,
                         artifact: DependencyArtifactDescriptor,
                         excludes: Seq[ExcludeRule]): Option[Elem] = {
  ...
  /**
    ...
    */
  private def depsInConfs(module: ModuleDescriptor,
                          configurations: Option[Iterable[Configuration]]): Seq[DependencyDescriptor] = {
