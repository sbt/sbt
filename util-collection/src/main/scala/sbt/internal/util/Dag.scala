...
object Dag:
  ...
  def topologicalSort[T](nodes: Iterable[T])(dependencies: T => Iterable[T]): List[T] =
    ...
    def visit(node: T): Unit =
      if !discovered(node) then
        discovered(node) = true;
        try visitAll(dependencies(node))
        catch case c: Cyclic => throw node :: c
        finished += node
        ()
      else if !finished(node) then throw new Cyclic(node)
  ...
  final class Cyclic(val value: Any, val all: List[Any], val complete: Boolean)
      extends Exception(
        "Cyclic reference involving " +
          (if complete then all.mkString("\n   ", "\n   ", "") else value)
      ):
    def this(value: Any) = this(value, value :: Nil, false)
    override def toString = getMessage

    def ::(a: Any): Cyclic =
      if complete then this
      else if a == value then new Cyclic(value, all, true)
      else new Cyclic(value, a :: all, false)
