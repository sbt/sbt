...
def minimalSettings: Seq[Setting[_]] = commonSettings ++ customCommands ++ publishPomSettings
...
lazy val altLocalResolver = Resolver.file(
  altLocalRepoName,
  file(sys.props("user.home") + "/.ivy2/sbt-alternative")
...
lazy val interfaceProj = (project in file("interface")).settings(
  minimalSettings,
  javaOnlySettings,
  name := "Interface",
  projectComponent,
  ...
  apiDefinitions <<= baseDirectory map { base =>
    (base / "definition") :: (base / "other") :: (base / "type") :: Nil
  },
...
lazy val controlProj = (project in utilPath / "control").settings(
  baseSettings,
  Util.crossBuild,
  name := "Control",
...
lazy val collectionProj = (project in utilPath / "collection").settings(
  testedBaseSettings,
...
// cross versioning
lazy val crossProj = (project in utilPath / "cross").settings(
  baseSettings,
...
lazy val compileInterfaceProj = (project in compilePath / "interface")
  .dependsOn(
    interfaceProj % "compile;test->test",
    ioProj        % "test->test",
    logProj       % "test->test", /*launchProj % "test->test",*/ apiProj % "test->test"
  )
  .settings(
    baseSettings,
...
lazy val packageBridgeSource =
  settingKey[Boolean]("Whether to package the compiler bridge sources in compiler ivy project's resources.")
  ...
  .settings(
    ...
    resourceGenerators in Compile <+= Def.task {
      if (packageBridgeSource.value) {
        ...
        val scalaJars =
          (externalDependencyClasspath in Compile).value.map(_.data).filter(j => included contains j.getName)
...
def allProjects =
  Seq(
    ...
    sbtProj,
    bundledLauncherProj,
    mavenResolverPluginProj
  )

def projectsWithMyProvided =
  allProjects.map(p => p.copy(configurations = (p.configurations.filter(_ != Provided)) :+ myProvided))
...
def otherRootSettings =
  ...
  ) ++ inConfig(Scripted.MavenResolverPluginTest)(
    Seq(
      Scripted.scriptedLaunchOpts := List("-XX:MaxPermSize=256M", "-Xmx1G"),
      Scripted.scripted <<= scriptedTask,
      Scripted.scriptedUnpublished <<= scriptedUnpublishedTask,
