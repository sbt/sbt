...
private[sbt] object Analyze {
  def apply[T](
      ...
      sources: Seq[File],
      log: Logger
  )(analysis: xsbti.AnalysisCallback, loader: ClassLoader, readAPI: (File, Seq[Class[_]]) => Set[String]) {
    ...
    for ((source, classFiles) <- sourceToClassFiles) {
      val publicInherited = readAPI(
        source,
        classFiles.toSeq.flatMap(c => load(c.className, Some("Error reading API from class file")))
      )
      ...
      def processDependencies(tpes: Iterable[String], context: DependencyContext): Unit =
        tpes.foreach(tpe => processDependency(tpe, context))
  ...
  private def isMain(modifiers: Int): Boolean =
    (modifiers & mainModifiers) == mainModifiers && (modifiers & notMainModifiers) == 0
