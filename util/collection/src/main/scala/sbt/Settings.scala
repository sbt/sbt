...
private final class Settings0[Scope](val data: Map[Scope, AttributeMap], val delegates: Scope => Seq[Scope])
...
trait Init[Scope] {
  ...
  def app[K[L[x]], T](inputs: K[Initialize])(f: K[Id] => T)(implicit alist: AList[K]): Initialize[T] =
  ...
  /**
    ...
    */
  private[sbt] final def validated[T](key: ScopedKey[T], selfRefOk: Boolean): ValidationCapture[T] =
  ...
  private final class InvalidReference(val key: ScopedKey[_])
      extends RuntimeException("Internal settings error: invalid reference to " + showFullKey(key))
  ...
  def compiled(init: Seq[Setting[_]], actual: Boolean = true)(implicit delegates: Scope => Seq[Scope],
  ...
  def make(init: Seq[Setting[_]])(implicit delegates: Scope => Seq[Scope],
    ...
    try { applyInits(ordered) } catch {
      case rru: RuntimeUndefined => throw Uninitialized(cMap.keys.toSeq, delegates, rru.undefined, true)
    }
  }
  ...
  private[this] def applyInits(
      ordered: Seq[Compiled[_]]
  ...
  def showUndefined(u: Undefined, validKeys: Seq[ScopedKey[_]], delegates: Scope => Seq[Scope])(
    ...
    val guessedString =
      if (derived) "" else guessed.map(g => "\n     Did you mean " + display(g) + " ?").toList.mkString
    val derivedString =
      if (derived) ", which is a derived setting that needs this key to be defined in this scope." else ""
  ...
  final class Undefined private[sbt] (val defining: Setting[_], val referencedKey: ScopedKey[_]) {
    ...
    def this(definingKey: ScopedKey[_], referencedKey: ScopedKey[_], derived: Boolean) =
  ...
  private[this] def deriveAndLocal(init: Seq[Setting[_]])(implicit delegates: Scope => Seq[Scope],
  ...
  sealed trait Initialize[T] {
    ...
    def validateReferenced(g: ValidateRef): ValidatedInit[T] =
      validateKeyReferenced(
        new ValidateKeyRef { def apply[T](key: ScopedKey[T], selfRefOk: Boolean) = g(key) }
      )
  ...
  sealed class Setting[T] private[Init] (val key: ScopedKey[T],
    ...
    protected[this] def make[T](key: ScopedKey[T], init: Initialize[T], pos: SourcePosition): Setting[T] =
      new Setting[T](key, init, pos)
  ...
  private[Init] sealed class DerivedSetting[T](sk: ScopedKey[T],
    ...
    override def make[T](key: ScopedKey[T], init: Initialize[T], pos: SourcePosition): Setting[T] =
  ...
  private[sbt] final class Apply[K[L[x]], T](val f: K[Id] => T, val inputs: K[Initialize], val alist: AList[K])
      extends Initialize[T] {
