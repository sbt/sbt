...
sealed trait RichParser[A] {
  ...
  /**
    ...
    */
  def examples(exampleSource: ExampleSource,
               maxNumberOfExamples: Int,
               removeInvalidExamples: Boolean): Parser[A]
...
object Parser extends ParserMain {
  ...
  @deprecated(
    "This method is deprecated and will be removed in the next major version. Use the parser directly to check for invalid completions.",
    since = "0.13.2"
  )
  ...
  private[complete] def repeat[T](partial: Option[Parser[T]],
                                  repeated: Parser[T],
                                  min: Int,
                                  max: UpperBound,
                                  revAcc: List[T]): Parser[Seq[T]] = {
      ...
      repeated match {
        ...
        case _ =>
          repeated.result match {
            ...
            case None =>
              if (max.isZero) success(revAcc.reverse) else new Repeat(partial, repeated, min, max, revAcc)
          }
      }
...
trait ParserMain {
  ...
  implicit def richParser[A](a: Parser[A]): RichParser[A] = new RichParser[A] {
    ...
    def ^^^[B](value: B): Parser[B] = a map { _ =>
      value
    }
    ...
    def examples(s: ExampleSource, maxNumberOfExamples: Int, removeInvalidExamples: Boolean): Parser[A] =
...
private final class BindParser[A, B](a: Parser[A], f: A => Parser[B]) extends ValidParser[B] {
  lazy val result = a.result flatMap { av =>
    f(av).result
  }
  lazy val resultEmpty = a.resultEmpty flatMap { av =>
    f(av).resultEmpty
  }
...
/**
  ...
  */
private final class ParserWithExamples[T](delegate: Parser[T],
...
private final class Repeat[T](partial: Option[Parser[T]],
                              repeated: Parser[T],
                              min: Int,
                              max: UpperBound,
  ...
  def repeatDerive(c: Char, accRev: List[T]): Parser[Seq[T]] =
    repeat(Some(repeated derive c), repeated, (min - 1) max 0, max.decrement, accRev)
