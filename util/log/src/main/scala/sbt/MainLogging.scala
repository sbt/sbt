...
object MainLogging {
  ...
  def defaultMultiConfig(console: ConsoleOut, backing: AbstractLogger): MultiLoggerConfig =
    new MultiLoggerConfig(
      defaultScreen(console, ConsoleLogger.noSuppressedMessage),
      backing,
      Nil,
      Level.Info,
      Level.Debug,
  ...
  @deprecated("Explicitly specify the console output.", "0.13.0")
  def defaultScreen(suppressedMessage: SuppressedTraceContext => Option[String]): AbstractLogger =
    ConsoleLogger(suppressedMessage = suppressedMessage)

  def defaultScreen(console: ConsoleOut): AbstractLogger = ConsoleLogger(console)
  def defaultScreen(console: ConsoleOut,
                    suppressedMessage: SuppressedTraceContext => Option[String]): AbstractLogger =
...
final case class MultiLoggerConfig(console: AbstractLogger,
                                   backed: AbstractLogger,
                                   extra: List[AbstractLogger],
