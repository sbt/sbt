...
class DatatypeParser extends NotNull {
  ...
  def processMemberLine(l: Array[String], line: Int) =
    new MemberLine(l(1), l(2), l(3).isEmpty, l(0).length, line)
  ...
  def parseLine(line: String, lineNumber: Int): Line =
    matchPattern(
      WhitespacePattern -> processWhitespaceLine _,
      EnumPattern -> processEnumLine _,
      ClassPattern -> processClassLine _,
      MemberPattern -> processMemberLine _
    )(line, lineNumber)
  ...
  def processLine(open: Array[ClassDef],
                  definitions: List[Definition],
                  line: Line): (Array[ClassDef], List[Definition]) = {
    ...
    line match {
      ...
      case m: MemberLine =>
        if (m.level == 0 || m.level > open.length) error(m, "Member must be declared in a class definition")
          ...
          withCurrent(open, definitions, m.level) { c =>
            List(c + m)
          }
      case c: ClassLine =>
        if (c.level == 0) (Array(new ClassDef(c.name, None, Nil, false)), open.toList ::: definitions)
          ...
          withCurrent(open, definitions, c.level) { p =>
            val p1 = makeAbstract(p); p1 :: new ClassDef(c.name, Some(p1), Nil, false) :: Nil
          }
    }
  }
  private def withCurrent(open: Array[ClassDef], definitions: List[Definition], level: Int)(
      onCurrent: ClassDef => Seq[ClassDef]
  ): (Array[ClassDef], List[Definition]) = {
