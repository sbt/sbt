...
abstract class GeneratorBase(val basePkgName: String, val baseDirectory: File) {
  ...
  def writeDefinition(d: Definition) = d match {
    case e: EnumDef => writeEnum(e); case c: ClassDef => writeClass(c)
  }
...
class ImmutableGenerator(pkgName: String, baseDir: File) extends GeneratorBase(pkgName, baseDir) {
  ...
  def classContent(c: ClassDef): String = {
    ...
    val accessors = normalizedMembers.map(
      m => "public final " + m.asJavaDeclaration(true) + "()\n\t{\n\t\treturn " + m.asGet + ";\n\t}"
    )
    ...
    "import java.util.Arrays;\n" +
      ...
      "public " + abstractStr + "class " + c.name + c.parent
      .map(" extends " + _.name + " ")
      .getOrElse(" implements java.io.Serializable") + "\n" +
      "{\n\t" +
      constructor + "\n\t" +
...
object Generator {
  ...
  def equalsMethod(c: ClassDef): String = {
    val content = if (c.hasLazyMembers) {
      ...
      val allMembers = c.allMembers.map(normalize)
      val memberComparisons =
        if (allMembers.isEmpty) "true"
        else allMembers.map(m => fieldEquals(m.name + "()", m.single, isPrimitive(m.tpe))).mkString(" && ")
      ...
      if (m.single) {
        if (primitive) {
          m.tpe.toLowerCase match {
            ...
            case "double" =>
              "(int)(Double.doubleToLongBits(" + f + ") ^ (Double.doubleToLongBits(" + f + ") >>> 32))"
            case "int" => f
            case _     => "(int)" + f
          }
        } else {
  ...
  def toStringMethod(c: ClassDef): String = {
    val content = if (c.hasLazyMembers) {
      ...
      val parametersString =
        ...
        else allMembers.map(m => fieldToString(m.name, m.single)).mkString(" + \", \" + ")
