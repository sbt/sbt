...
object ContextUtil {
  ...
  def unexpectedTree[C <: Context](tree: C#Tree): Nothing =
...
final class ContextUtil[C <: Context](val ctx: C) {
    ...
    val process = new Traverser {
      override def traverse(t: Tree) = t match {
        ...
        case ApplyTree(TypeApply(Select(_, nme), tpe :: Nil), qual :: Nil)
            if isWrapper(nme.decoded, tpe.tpe, qual) =>
          ()
        case tree =>
  ...
  def functionType(args: List[Type], result: Type): Type = {
    val tpe =
      global.definitions.functionType(args.asInstanceOf[List[global.Type]], result.asInstanceOf[global.Type])
  ...
  def changeOwner(tree: Tree, prev: Symbol, next: Symbol): Unit =
    new ChangeOwnerAndModuleClassTraverser(prev.asInstanceOf[global.Symbol], next.asInstanceOf[global.Symbol])
  ...
  private[this] class ChangeOwnerAndModuleClassTraverser(oldowner: global.Symbol, newowner: global.Symbol)
