...
object Instance {
  ...
  /**
    ...
    */
  def contImpl[T, N[_]](c: Context, i: Instance with Singleton, convert: Convert, builder: TupleBuilder)(
  ...
  implicit def applicativeInstance[A[_]](implicit ap: Applicative[A]): Instance { type M[x] = A[x] } =
    new Instance {
      type M[x] = A[x]
