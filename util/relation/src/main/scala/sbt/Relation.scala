...
private final class MRelation[A, B](fwd: Map[A, Set[B]], rev: Map[B, Set[A]]) extends Relation[A, B] {
  ...
  def +(from: A, to: Traversable[B]) =
    if (to.isEmpty) this
      ...
      new MRelation(add(fwd, from, to), (rev /: to) { (map, t) =>
        add(map, t, from :: Nil)
      })
  ...
  def groupBy[K](discriminator: ((A, B)) => K): Map[K, Relation[A, B]] = all.groupBy(discriminator) mapValues {
    Relation.empty[A, B] ++ _
  }
