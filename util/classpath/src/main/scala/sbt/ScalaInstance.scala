...
/**
  ...
  */
final class ScalaInstance(val version: String,
                          val loader: ClassLoader,
                          val libraryJar: File,
                          @deprecated(
                            "Only `allJars` and `jars` can be reliably provided for modularized Scala.",
                            "0.13.0"
  ...
  override def toString =
    "Scala instance{version label " + version + ", actual version " + actualVersion + ", " + jarStrings + "}"
}
object ScalaInstance {
  ...
  def apply(org: String, version: String, launcher: xsbti.Launcher): ScalaInstance =
    ...
    if (org == ScalaOrg)
      ...
      try {
        ...
        case x: NoSuchMethodError =>
          sys.error("Incompatible version of the xsbti.Launcher interface. Use an sbt 0.12+ launcher instead.")
      }
  ...
  def apply(version: String, provider: xsbti.ScalaProvider): ScalaInstance =
    new ScalaInstance(
      ...
      provider.loader,
      provider.libraryJar,
      provider.compilerJar,
      (provider.jars.toSet - provider.libraryJar - provider.compilerJar).toSeq,
      None
    )
  ...
  @deprecated("Does not handle modularized Scala.  Use a variant that only accepts all jars.", "0.13.0")
  def apply(libraryJar: File, compilerJar: File, extraJars: File*)(
  ...
  def apply(version: String,
            ...
            extraJars: File*): ScalaInstance =
    apply(version, None, libraryJar, compilerJar, launcher, extraJars: _*)

  @deprecated("Does not handle modularized Scala.  Use a variant that only accepts all jars.", "0.13.0")
  def apply(version: String, libraryJar: File, compilerJar: File, extraJars: File*)(
      classLoader: List[File] => ClassLoader
  ): ScalaInstance =
  ...
  def apply(version: String,
            ...
            libraryJar: File,
            compilerJar: File,
            extraJars: File*)(classLoader: List[File] => ClassLoader): ScalaInstance =
    new ScalaInstance(
      version,
      classLoader(libraryJar :: compilerJar :: extraJars.toList),
      libraryJar,
      compilerJar,
      extraJars,
      explicitActual
  ...
  private def scalaLoader(launcher: xsbti.Launcher): Seq[File] => ClassLoader =
    jars => new URLClassLoader(jars.map(_.toURI.toURL).toArray[URL], launcher.topLoader)
}
