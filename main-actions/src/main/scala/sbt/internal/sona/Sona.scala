...
class SonaClient(reqTransform: Request => Request, uploadRequestTimeout: FiniteDuration)
  ...
  def waitForDeploy(
    ...
    val sleepSec =
      if attempt <= 3 then List(5, 5, 10, 15)(attempt)
      else 30
  ...
  private def awaitWithMessage[A1](
    ...
    def logLoop(attempt: Int): Unit =
      if !f.isCompleted then
        if attempt > 0 then log.info(msg)
...
object SonaClient:
  ...
  val asStringWithErrorBody: FullResponse => String = response =>
    ...
    if response.status >= 200 && response.status < 300 then body
...
class SonaStatusError(val status: Int, val body: String)
    extends RuntimeException(
      if body.nonEmpty then s"Unexpected status: $status\n$body"
