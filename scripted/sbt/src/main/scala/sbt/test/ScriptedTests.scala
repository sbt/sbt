          ...
          testResources.readWriteResourceDirectory(g, n) { testDirectory =>
            ...
            if (disabled) {
              ...
              try { scriptedTest(str, testDirectory, prescripted, log); None } catch {
                case _: xsbt.test.TestException | _: PendingTestSuccessException => Some(str)
              }
            }
          }
        }
...
class ScriptedRunner {
  ...
  def run(resourceBaseDirectory: File,
    ...
    run(resourceBaseDirectory, bufferLog, tests, ConsoleLogger(), bootProperties, launchOpts, { f: File =>
      prescripted.add(f); ()
