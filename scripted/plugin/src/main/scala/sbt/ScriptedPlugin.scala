...
object ScriptedPlugin extends Plugin {
  ...
  val scriptedLaunchOpts =
    SettingKey[Seq[String]]("scripted-launch-opts", "options to pass to jvm launching scripted tasks")
  ...
  val scriptedSettings = Seq(
    ...
    scriptedDependencies <<= (compile in Test, publishLocal) map { (analysis, pub) =>
      Unit
    },
