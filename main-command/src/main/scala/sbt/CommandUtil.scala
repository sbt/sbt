...
object CommandUtil:
  ...
  def processLine(s: String): Option[String] =
    val s2 = s.trim; if ignoreLine(s2) then None else Some(s2)
    ...
    detailMap.get(selected) match
      ...
      case None              =>
        try
          ...
          if details.isEmpty then "No matches for regular expression '" + selected + "'."
  ...
  def searchHelp(selected: String, detailMap: Map[String, String]): Map[String, String] =
    ...
    detailMap flatMap { (k, v) =>
      ...
      if keyMatches.isDefined || contentMatches.isDefined then Seq((keyString, contentString))
      else nilSeq
    }
