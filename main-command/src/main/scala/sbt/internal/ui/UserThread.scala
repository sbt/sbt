...
private[sbt] class UserThread(val channel: CommandChannel) extends AutoCloseable:
  ...
  private[sbt] def reset(state: State): Unit = if !isClosed.get then
  ...
  private[sbt] def onConsolePromptEvent(consolePromptEvent: ConsolePromptEvent): Unit =
    ...
    // at the bottom
    synchronized {
      if terminal.isAnsiSupported then
  ...
  private[sbt] def onConsoleUnpromptEvent(
    ...
    if consoleUnpromptEvent.lastSource.fold(true)(_.channelName != name) then
      terminal.progressState.reset()
    else stopThread()

  override def close(): Unit = if isClosed.compareAndSet(false, true) then executor.shutdown()
