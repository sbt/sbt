...
trait JavaCompiler extends xsbti.compile.JavaCompiler {
  ...
  def compileWithReporter(sources: Array[File],
                          classpath: Array[File],
                          output: Output,
                          options: Array[String],
                          reporter: Reporter,
...
object JavaCompiler {
  ...
  val javadoc = new JavacContract("javadoc", "com.sun.tools.javadoc.Main") {
    def exec(args: Array[String], writer: PrintWriter) = {
      ...
      m.invoke(null, name, writer, writer, writer, "com.sun.tools.doclets.standard.Standard", args)
  ...
  private[this] class JavaTool0(f: Fork,
    ...
    def commandArguments(sources: Seq[File],
      ...
      (new CompilerArguments(scalaInstance, javaCp))(
        sources,
        augmentedClasspath,
    ...
    def compile(contract: JavacContract,
      ...
      if (code != 0)
        throw new CompileFailed(arguments.toArray, contract.name + " returned nonzero exit code", Array())
    }
  }
  ...
  def directOrForkJavac(implicit doFork: Fork) =
    (contract: JavacContract, arguments: Seq[String], log: Logger) =>
      try { directJavac(contract, arguments, log) } catch {
        case e @ (_: ClassNotFoundException | _: NoSuchMethodException) =>
          log.debug(
            contract.clazz + " not found with appropriate method signature; forking " + contract.name + " instead"
          )
  ...
  val directJavac = (contract: JavacContract, arguments: Seq[String], log: Logger) => {
    ...
    var exitCode = -1
    try { exitCode = contract.exec(argsArray, writer) } finally {
      logger.flushLines(if (exitCode == 0) Level.Warn else Level.Error)
    }
    exitCode
  }
