...
/**
  ...
  */
final class AnalyzingCompiler private (val scalaInstance: xsbti.compile.ScalaInstance,
                                       val provider: CompilerInterfaceProvider,
                                       val cp: xsbti.compile.ClasspathOptions,
                                       onArgsF: Seq[String] => Unit,
                                       val classLoaderCache: Option[ClassLoaderCache])
    extends CachedCompilerProvider {
  def this(scalaInstance: xsbti.compile.ScalaInstance,
           provider: CompilerInterfaceProvider,
           cp: xsbti.compile.ClasspathOptions) =
    this(scalaInstance, provider, cp, _ => (), None)
  def this(scalaInstance: ScalaInstance, provider: CompilerInterfaceProvider) =
    this(scalaInstance, provider, ClasspathOptions.auto)

  @deprecated("A Logger is no longer needed.", "0.13.0")
  def this(scalaInstance: ScalaInstance, provider: CompilerInterfaceProvider, log: Logger) =
    this(scalaInstance, provider)

  @deprecated("A Logger is no longer needed.", "0.13.0")
  def this(scalaInstance: xsbti.compile.ScalaInstance,
           provider: CompilerInterfaceProvider,
           cp: xsbti.compile.ClasspathOptions,
           log: Logger) = this(scalaInstance, provider, cp)
  ...
  def apply(sources: Seq[File],
    ...
    compile(
      ...
      output,
      callback,
      new LoggerReporter(maximumErrors, log, p => p),
      cache,
      log,
      None
    )
  ...
  def newCachedCompiler(arguments: Array[String],
                        output: Output,
                        log: xLogger,
                        reporter: Reporter,
  ...
  def newCachedCompiler(arguments: Seq[String],
                        output: Output,
                        log: xLogger,
                        reporter: Reporter,
                        resident: Boolean): CachedCompiler =
  ...
  def doc(sources: Seq[File],
    ...
    val arguments =
      (new CompilerArguments(scalaInstance, cp))(sources, classpath, Some(outputDirectory), options)
    onArgsF(arguments)
    call("xsbt.ScaladocInterface", "run", log)(classOf[Array[String]], classOf[xLogger], classOf[Reporter])(
      arguments.toArray[String]: Array[String],
      log,
      reporter
    )
  }
  def console(classpath: Seq[File],
              options: Seq[String],
              initialCommands: String,
              cleanupCommands: String,
              log: Logger)(loader: Option[ClassLoader] = None, bindings: Seq[(String, Any)] = Nil): Unit = {
  ...
  private def call(interfaceClassName: String, methodName: String, log: Logger)(
      argTypes: Class[_]*
  )(args: AnyRef*): AnyRef = {
...
object AnalyzingCompiler {
  import sbt.IO.{ copy, createDirectory, jars, unzip, withTemporaryDirectory, zip }
  ...
  /**
    ...
    */
  def compileSources(sourceJars: Iterable[File],
                     targetJar: File,
    ...
    withTemporaryDirectory { dir =>
      ...
      withTemporaryDirectory { outputDirectory =>
        log.info(
          "'" + id + "' not yet compiled for Scala " + compiler.scalaInstance.actualVersion + ". Compiling..."
        )
        val start = System.currentTimeMillis
        try {
          compiler(
            sourceFiles.toSeq,
            compiler.scalaInstance.libraryJar +: (xsbtiJars.toSeq ++ sourceJars),
            outputDirectory,
            "-nowarn" :: Nil
          )
          ...
          case e: xsbti.CompileFailed =>
            throw new CompileFailed(e.arguments, "Error compiling sbt component '" + id + "'", e.problems)
        }
        import sbt.Path._
