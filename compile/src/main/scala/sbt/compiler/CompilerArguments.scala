...
final class CompilerArguments(scalaInstance: xsbti.compile.ScalaInstance, cp: xsbti.compile.ClasspathOptions) {
  def apply(sources: Seq[File],
    ...
    val outputOption = outputDirectory map { out =>
      Seq("-d", out.getAbsolutePath)
    } getOrElse Seq()
  ...
  def finishClasspath(classpath: Seq[File]): Seq[File] =
    filterLibrary(classpath) ++ include(cp.compiler, scalaInstance.compilerJar) ++ include(
      cp.extra,
      scalaInstance.otherJars: _*
    )
  ...
  def createBootClasspathFor(classpath: Seq[File]) =
    createBootClasspath(hasLibrary(classpath) || cp.compiler || cp.extra)
  ...
  def filterLibrary(classpath: Seq[File]) =
    if (cp.filterLibrary) classpath filterNot isScalaLibrary else classpath
  ...
  def bootClasspathOption(addLibrary: Boolean) =
    if (cp.autoBoot) Seq(BootClasspathOption, createBootClasspath(addLibrary)) else Nil
  def bootClasspath(addLibrary: Boolean) =
    if (cp.autoBoot) IO.parseClasspath(createBootClasspath(addLibrary)) else Nil
