...
object ShowAPI {
  ...
  def showApi(a: SourceAPI)(implicit nesting: Int) =
    a.packages.map(pkg => "package " + pkg.name).mkString("\n") + lines(
      truncateDecls(a.definitions).map(showDefinition)
    )
  ...
  def showDefinition(d: Definition)(implicit nesting: Int): String = d match {
    ...
    case cl: ClassLike =>
      showPolyDef(cl, showDefinitionType(cl.definitionType)) + " extends " + showTemplate(cl)
  }
  ...
  private def showTemplate(cl: ClassLike)(implicit nesting: Int) =
    if (nesting <= 0) "<nesting level reached>"
      ...
      val showSelf =
        if (cl.selfType.isInstanceOf[EmptyType]) "" else " self: " + showNestedType(cl.selfType) + " =>"
  ...
  def showType(t: Type)(implicit nesting: Int): String = t match {
    ...
    case s: Structure =>
      s.parents.map(showType).mkString(" with ") + (if (nesting <= 0) "{ <nesting level reached> }"
                                                    else
                                                      truncateDecls(s.declared)
                                                        .map(showNestedDefinition)
                                                        .mkString(" {", "\n", "}"))
    case e: Existential =>
      showType(e.baseType) + (if (nesting <= 0) " forSome { <nesting level reached> }"
                              else
                                e.clause
                                  .map(t => "type " + showNestedTypeParameter(t))
                                  .mkString(" forSome { ", "; ", " }"))
    case p: Polymorphic =>
      showType(p.baseType) + (if (nesting <= 0) " [ <nesting level reached> ]"
  ...
  private def showMonoDef(d: Definition, label: String)(implicit nesting: Int): String =
    space(showAnnotations(d.annotations)) + space(showAccess(d.access)) + space(showModifiers(d.modifiers)) + space(
      label
    ) + d.name
