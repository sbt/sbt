...
@deprecated(
  "This class is not used in incremental compiler and will be removed in next major version.",
  "0.13.2"
)
class NameChanges(val newTypes: Set[String],
                  val removedTypes: Set[String],
                  val newTerms: Set[String],
                  val removedTerms: Set[String]) {
  override def toString =
    (("New types", newTypes) :: ("Removed types", removedTypes) :: ("New terms", newTerms) :: (
      "Removed terms",
      removedTerms
    ) :: Nil)
      .map {
...
class SameAPI(includePrivate: Boolean, includeParamNames: Boolean) {
  ...
  def sameNamedDefinitions(a: List[Definition], b: List[Definition]): Boolean = {
    ...
    debug((a.length == b.length), "\t\tLength differed for " + a.headOption.map(_.name).getOrElse("empty")) && sameDefs(
      a,
      b
    )
  }
  ...
  def sameQualifier(a: Qualifier, b: Qualifier): Boolean =
    (a, b) match {
      ...
      case _ =>
        debug(false, "Different qualifier categories: " + a.getClass.getName + " -- " + b.getClass.getName)
    }
  ...
  def sameTypeDirect(a: Type, b: Type): Boolean =
    (a, b) match {
      case (sa: SimpleType, sb: SimpleType) =>
        debug(
          sameSimpleTypeDirect(sa, sb),
          "Different simple types: " + DefaultShowAPI(sa) + " and " + DefaultShowAPI(sb)
        )
