...
object ClassToAPI {
  ...
  final class ClassMap private[sbt] (private[sbt] val memo: mutable.Map[String, Seq[api.ClassLike]],
                                     private[sbt] val inherited: mutable.Set[Class[_]],
  ...
  def structure(c: Class[_], enclPkg: Option[String], cmap: ClassMap): (api.Structure, api.Structure) = {
    ...
    val instanceStructure =
      new api.Structure(lzyS(parentTypes.toArray), lzyS(all.declared.toArray), lzyS(all.inherited.toArray))
    val staticStructure =
      new api.Structure(lzyEmptyTpeArray, lzyS(all.staticDeclared.toArray), lzyS(all.staticInherited.toArray))
  ...
  def exceptionAnnotations(exceptions: Array[Type]): Array[api.Annotation] =
    if (exceptions.length == 0) emptyAnnotationArray
      ...
      arrayMap(exceptions)(
        t => new api.Annotation(Throws, Array(new api.AnnotationArgument("value", t.toString)))
      )
  ...
  case class Defs(declared: Seq[api.Definition],
                  inherited: Seq[api.Definition],
                  staticDeclared: Seq[api.Definition],
                  staticInherited: Seq[api.Definition]) {
    def ++(o: Defs) =
      Defs(
        declared ++ o.declared,
  ...
  def access(i: Int, pkg: Option[String]): api.Access = {
    import Modifier.{ isPrivate, isProtected, isPublic }
  ...
  private[this] def superclassType(c: Class[_]): Type =
    try c.getGenericSuperclass
    catch {
      case _: GenericSignatureFormatError => c.getSuperclass
    }
  private[this] def interfaces(c: Class[_]): Array[Type] =
    try c.getGenericInterfaces
    catch {
