...
class DependencySpecification extends Specification {
	...
	"Extracted source dependencies from public members" should {
		...
		memberRef('H) === Set('G, 'E, 'B)
		// aliases and applied type constructors are expanded so we have inheritance dependency on B
		inheritance('H) === Set('E, 'B)
	}

	"Extracted source dependencies from private members" should {
