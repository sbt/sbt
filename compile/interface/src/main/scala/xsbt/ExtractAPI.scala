...
class ExtractAPI[GlobalType <: CallbackGlobal](val global: GlobalType,
    ...
    staticAnnotations(as).toArray.map { a =>
      new xsbti.api.Annotation(
        ...
        if (a.assocs.isEmpty)
          ...
          a.assocs
            .map { case (name, value) => new xsbti.api.AnnotationArgument(name.toString, value.toString) }
  ...
  private def printMember(label: String, in: Symbol, t: Type) =
    println(label + " in " + in + " : " + t + " (debug: " + debugString(t) + " )")
  ...
  private def fieldDef[T](
      ...
      s: Symbol,
      keepConst: Boolean,
      create: (xsbti.api.Type, String, xsbti.api.Access, xsbti.api.Modifiers, Array[xsbti.api.Annotation]) => T
  ): T = {
  ...
  private def getAccess(c: Symbol): xsbti.api.Access =
    if (c.isPublic) Constants.public
      ...
      val qualifier =
        if (within == NoSymbol) Constants.unqualified else new xsbti.api.IdQualifier(within.fullName)
  ...
  private def mkClassLike(in: Symbol, c: Symbol): ClassLike = {
    ...
    new xsbti.api.ClassLike(
      defType,
      lzy(selfType(in, sym)),
      lzy(structureWithInherited(viewer(in).memberInfo(sym), sym)),
      emptyStringArray,
      typeParameters(in, sym), // look at class symbol
      c.fullName,
      getAccess(c),
      getModifiers(c),
      annotations(in, c)
  ...
  private def staticAnnotations(annotations: List[AnnotationInfo]): List[AnnotationInfo] = {
    ...
    class IsStatic(ann: AnnotationInfo) {
      def isStatic: Boolean = ann.atp.typeSymbol isNonBottomSubClass definitions.StaticAnnotationClass
    }
