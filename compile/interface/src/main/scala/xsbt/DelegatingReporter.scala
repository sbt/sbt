...
private final class DelegatingReporter(warnFatal: Boolean,
  ...
  private[this] def position(sourcePath0: Option[String],
  ...
  private[this] def o2mi(opt: Option[Int]): Maybe[I] = opt match {
    case None => Maybe.nothing[I]; case Some(s) => Maybe.just[I](s)
  }
  private[this] def o2m[S](opt: Option[S]): Maybe[S] = opt match {
    case None => Maybe.nothing[S]; case Some(s) => Maybe.just(s)
  }
}
