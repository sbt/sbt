...
object TestCaseGenerators {
  ...
  def genStamps(rel: Relations): Gen[Stamps] = {
    ...
    for {
      ...
      Stamps(
        zipMap(prod, prodStamps),
        zipMap(src, srcStamps),
        zipMap(bin, binStamps),
        zipMap(bin, binClassNames)
      )
  }
  ...
  private[this] def makeDefinition(name: String): Definition =
    new ClassLike(
      ...
      lzy(new EmptyType()),
      lzy(new Structure(lzy(Array()), lzy(Array()), lzy(Array()))),
      Array(),
      Array(),
      name,
      new Public(),
  ...
  def genNameHashes(defns: Seq[String]): Gen[xsbti.api._internalOnly_NameHashes] = {
    ...
    for {
      ...
      new xsbti.api._internalOnly_NameHashes(regularMemberNameHashes.toArray, implicitMemberNameHashes.toArray)
  }

  def genSource(defns: Seq[String]): Gen[Source] =
    for {
      ...
      new Source(
        new Compilation(startTime, Array()),
        hash,
        new SourceAPI(Array(), Array(defns map makeDefinition: _*)),
        apiHash,
        nameHashes,
        hasMacro,
        hasPackageObject
  ...
  def genSources(all_defns: Seq[Seq[String]]): Gen[Seq[Source]] =
    Gen.sequence[List, Source](all_defns.map(genSource))
  ...
  def genAPIs(rel: Relations): Gen[APIs] = {
    ...
    for {
      internalSources <- genSources(internal map { f: File =>
        rel.classNames(f).toList.sorted
      })
      externalSources <- genSources(external map { s: String =>
        s :: Nil
      })
  ...
  def genRSource(srcs: List[File]): Gen[Relations.Source] =
    for {
      ...
      Relations.makeSource( // Ensure that we don't generate a dep of some file on itself.
        Relation.reconstruct((srcs zip (internal map { _.toSet }) map { case (a, b) => (a, b - a) }).toMap),
        external
      )

  def genSubRSource(src: Relations.Source): Gen[Relations.Source] =
    for {
  ...
  def genRSourceDependencies(srcs: List[File]): Gen[Relations.SourceDependencies] =
    for {
      internal <- listOfN(srcs.length, someOf(srcs))
      external <- genStringRelation(srcs)
    } yield
      Relations.makeSourceDependencies(Relation.reconstruct((srcs zip (internal map { _.toSet }) map {
        case (a, b) => (a, b - a)
      }).toMap), external)

  def genSubRSourceDependencies(src: Relations.SourceDependencies): Gen[Relations.SourceDependencies] =
    for {
      internal <- someOf(src.internal.all.toList)
      external <- someOf(src.external.all.toList)
    } yield Relations.makeSourceDependencies(Relation.empty ++ internal, Relation.empty ++ external)

  def genRelations: Gen[Relations] =
    for {
  ...
  def genAnalysis(nameHashing: Boolean): Gen[Analysis] =
    for {
