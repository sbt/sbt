...
object ComponentCompiler {
  ...
  def interfaceProvider(manager: ComponentManager): CompilerInterfaceProvider = new CompilerInterfaceProvider {
    def apply(scalaInstance: xsbti.compile.ScalaInstance, log: Logger): File = {
      ...
      val componentCompiler =
        new ComponentCompiler(new RawCompiler(scalaInstance, ClasspathOptions.auto, log), manager)
      log.debug(
        "Getting " + compilerInterfaceID + " from component compiler for Scala " + scalaInstance.version
      )
  ...
  def interfaceProvider(manager: ComponentManager,
                        ...
                        sourcesModule: ModuleID): CompilerInterfaceProvider = new CompilerInterfaceProvider {
    def apply(scalaInstance: xsbti.compile.ScalaInstance, log: Logger): File = {
      // this is the instance used to compile the interface component
      val componentCompiler = new IvyComponentCompiler(
        new RawCompiler(scalaInstance, ClasspathOptions.auto, log),
        manager,
        ivyConfiguration,
        sourcesModule,
        log
...
class ComponentCompiler(compiler: RawCompiler, manager: ComponentManager) {
  ...
  def apply(id: String): File =
    try { getPrecompiled(id) } catch { case _: InvalidComponent => getLocallyCompiled(id) }
...
/**
  ...
  * to the actualVersion field of the RawCompiler.
  */
private[compiler] class IvyComponentCompiler(compiler: RawCompiler,
    ...
    IO.withTemporaryDirectory { binaryDirectory =>
      ...
      buffered bufferQuietly {
        ...
        IO.withTemporaryDirectory { retrieveDirectory =>
          (updateUtil.update(updateUtil.getModule(sourcesModule), retrieveDirectory)(
            _.getName endsWith "-sources.jar"
          )) match {
            case Some(sources) =>
              AnalyzingCompiler.compileSources(
                sources,
