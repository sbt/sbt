...
final class MixedAnalyzingCompiler(val scalac: AnalyzingCompiler,
    ...
    def compileJava(): Unit =
      if (javaSrcs.nonEmpty) {
        ...
        timed("Java compile + analysis", log) {
          javac.compile(
            javaSrcs,
            options.javacOptions.toArray[String],
            output,
            callback,
            reporter,
            log,
...
object MixedAnalyzingCompiler {
  ...
  def staticCachedStore(cacheFile: File) =
    staticCache(cacheFile, AnalysisStore.sync(AnalysisStore.cached(FileBasedStore(cacheFile))))
