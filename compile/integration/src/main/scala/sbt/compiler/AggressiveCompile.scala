...
class AggressiveCompile(cacheFile: File) {
  ...
  def apply(compiler: AnalyzingCompiler,
    ...
    val setup = new CompileSetup(
      output,
      new CompileOptions(options, javacOptions),
      compiler.scalaInstance.actualVersion,
      compileOrder,
  ...
  def compile2(config: CompileConfiguration)(implicit log: Logger,
                                             equiv: Equiv[CompileSetup]): (Boolean, Analysis) = {
    import config._
  ...
  private[this] def logInputs(log: Logger, javaCount: Int, scalaCount: Int, outputDirs: Seq[File]): Unit = {
    ...
    if (combined.nonEmpty)
      log.info(
        combined
          .mkString("Compiling ", " and ", " to " + outputDirs.map(_.getAbsolutePath).mkString(",") + "...")
      )
  }
  private def extract(previous: Option[(Analysis, CompileSetup)],
                      incOptions: IncOptions): (Analysis, Option[CompileSetup]) =
    previous match {
...
object AggressiveCompile {
  ...
  def forkJavac(javaHome: Option[File]): JavaCompiler.Fork = {
    import Path._
    def exec(jc: JavacContract) = javaHome match {
      case None => jc.name; case Some(jh) => (jh / "bin" / jc.name).absolutePath
    }
