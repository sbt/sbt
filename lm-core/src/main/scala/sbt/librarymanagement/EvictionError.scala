      ...
      .flatMap { case (config, detail) =>
        ...
        winner match
          case Some(winner) =>
            ...
            val assumedScheme =
              if isNameScalaSuffixed(detail.name) then assumedVersionScheme
            ...
            if hasIncompatibleVersionForScheme then
              Some(
                (
                  EvictionErrorPair(
                    detail.name,
...
final class EvictionError private[sbt] (
  ...
  def run(): Unit =
    if incompatibleEvictions.nonEmpty then sys.error(toLines.mkString("\n"))
