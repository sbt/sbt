...
object ConflictWarning:
  ...
  private def processCrossVersioned(
    ...
    if crossMismatches.nonEmpty then
      val pre =
  ...
  def crossVersionMismatches(report: UpdateReport): Map[(String, String), Set[String]] =
    val mismatches = report.configurations.flatMap { confReport =>
      groupByRawName(confReport.allModules).view.mapValues { modules =>
        ...
        if differentFullNames.size > 1 then differentFullNames else Set.empty[String]
      }
    }
    mismatches.foldLeft(Map.empty[(String, String), Set[String]])(merge)
  private def merge[A, B](m: Map[A, Set[B]], b: (A, Set[B])): Map[A, Set[B]] =
    if b._2.isEmpty then m
    else m.updated(b._1, m.getOrElse(b._1, Set.empty) ++ b._2)
