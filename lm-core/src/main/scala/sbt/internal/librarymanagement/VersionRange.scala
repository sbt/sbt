...
object VersionRange:
  ...
  def versionSatisfiesRange(version: String, range: String): Boolean =
    if !isVersionRange(range) then
      // Not a range, just compare directly
      version == range
    else if range.contains(",") && !hasMavenVersionRange(range) then
        ...
        case Some((lowerBound, lowerInclusive, upperBound, upperInclusive)) =>
          val lowerOk = lowerBound match
            case Some(lb) =>
              ...
              if lowerInclusive then cmp >= 0 else cmp > 0
            case None => true
          val upperOk = upperBound match
            case Some(ub) =>
              val cmp = compareVersions(version, ub)
              if upperInclusive then cmp <= 0 else cmp < 0
            case None => true
          lowerOk && upperOk
        case None => false
    else false
  ...
  private def parseMavenRange(
    ...
    if trimmed.length < 2 then None
      ...
      if !Set('[', '(').contains(startChar) || !Set(']', ')').contains(endChar) then None
      else
        ...
        if commaIdx < 0 then
          ...
          val v = inner.trim
          if v.nonEmpty then Some((Some(v), true, Some(v), true))
          else None
        else
          ...
          Some(
            (
              if lower.nonEmpty then Some(lower) else None,
              lowerInclusive,
              if upper.nonEmpty then Some(upper) else None,
              upperInclusive
            )
          )
      end if
  ...
  private def compareVersions(v1: String, v2: String): Int =
    ...
    val numericComparison = (0 until maxLen).iterator
      .map { i =>
        val n1 = if i < nums1.length then nums1(i) else 0L
        val n2 = if i < nums2.length then nums2(i) else 0L
        n1.compare(n2)
      }
      .find(_ != 0)
    ...
    numericComparison match
      ...
      case None      =>
        ...
        if tags1.isEmpty && tags2.nonEmpty then 1
        else if tags1.nonEmpty && tags2.isEmpty then -1
        else
          ...
          val tagComparison = (0 until tagMaxLen).iterator
            .map { i =>
              val t1 = if i < tags1.length then tags1(i) else ""
              val t2 = if i < tags2.length then tags2(i) else ""
              t1.compare(t2)
            }
            .find(_ != 0)
  ...
  def stripMavenVersionRange(version: String): Option[String] =
    if isVersionRange(version) then
  ...
  def fromIvyToMavenVersion(revision: String): String =
    def plusRange(s: String, shift: Int = 0) =
      def pow(i: Int): Int = if i > 0 then 10 * pow(i - 1) else 1
    ...
    try
      revision match
        ...
        case rev if startSym(rev(0)) && stopSym(rev(rev.length - 1)) =>
          ...
          (if start == ']' then "(" else start.toString) + mid + (if stop == '[' then ")" else stop)
        case _ => revision
    catch
  ...
  end fromIvyToMavenVersion

  def hasMavenVersionRange(version: String): Boolean =
    if version.length <= 1 then false
