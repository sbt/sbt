...
lazy val many = (projectMatrix in file("many"))
  ...
  .addPlatforms(VirtualAxis.jvm, wasm)("2.13.18", "3.3.6")(Seq(check := ()))

// every shape that takes a row function also takes settings
lazy val extraSettings = (projectMatrix in file("extraSettings"))
  .addPlatforms(VirtualAxis.jvm)("2.13.18")(config)(Seq(check := ()))

lazy val cvExtra = (projectMatrix in file("cvExtra"))
  .addPlatforms(VirtualAxis.jvm)(CrossVersion.full, "2.13.18")(config)(Seq(check := ()))

lazy val axesExtra = (projectMatrix in file("axesExtra"))
  .addPlatforms(VirtualAxis.jvm)(CrossVersionAxes.abi("2.13.18"), config)(Seq(check := ()))
...
lazy val byCv = (projectMatrix in file("byCv"))
  .addPlatformsBy(VirtualAxis.jvm)(CrossVersion.full, "2.13.18")((_, v) => Seq(check := ()))

lazy val byExtra = (projectMatrix in file("byExtra"))
  .addPlatformsBy(VirtualAxis.jvm)("2.13.18")(config)((_, v) => Seq(check := ()))
