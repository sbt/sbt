...
lazy val bin = (projectMatrix in file("bin"))
  .addPlatforms(VirtualAxis.jvm)("2.13.18")(
    _.settings(check := assert(crossVersion.value != CrossVersion.full, "bin"))
  )
...
lazy val full = (projectMatrix in file("full"))
  .addPlatforms(VirtualAxis.jvm)(CrossVersion.full, "2.13.18")(
    _.settings(check := assert(crossVersion.value == CrossVersion.full, "full"))
  )
...
// the same factories with a cross version, and the row asks for it in its settings
lazy val byCv = (projectMatrix in file("byCv"))
  .addPlatforms(VirtualAxis.jvm)(CrossVersion.full.axesBy(_.take(1))("2.13.18"))(
    _.settings(check := assert(crossVersion.value == CrossVersion.full, "byCv"))
  )
lazy val partialCv = (projectMatrix in file("partialCv"))
  .addPlatforms(VirtualAxis.jvm)(CrossVersion.full.axesPartial("3.3.6"))(
    _.settings(check := assert(crossVersion.value == CrossVersion.full, "partialCv"))
  )
lazy val byFactory = (projectMatrix in file("byFactory"))
  .addPlatforms(VirtualAxis.jvm)(CrossVersionAxes.by(CrossVersion.full)(_.take(1))("2.13.18"))(
    _.settings(check := assert(crossVersion.value == CrossVersion.full, "byFactory"))
  )
lazy val partialFactory = (projectMatrix in file("partialFactory"))
  .addPlatforms(VirtualAxis.jvm)(CrossVersionAxes.partial(CrossVersion.full, "3.3.6"))(
    _.settings(check := assert(crossVersion.value == CrossVersion.full, "partialFactory"))
  )
...
lazy val java = (projectMatrix in file("java")).addPlatforms(VirtualAxis.jvm)(
  CrossVersionAxes()
)(_.settings(check := assert(!autoScalaLibrary.value, "java")))

// a cross version without a version: one row, no Scala library, and the cross version set
lazy val javaCv = (projectMatrix in file("javaCv"))
  .addPlatforms(VirtualAxis.jvm)(CrossVersion.full)(
    _.settings(
      check := {
        assert(!autoScalaLibrary.value, "javaCv")
        assert(crossVersion.value == CrossVersion.full, "javaCv")
      }
    )
  )
  ...
  .settings(
    check := {
      val matrices = Seq(bin, full, patch, fixed, part, one, many, mixed, abi, partial, whole,
        byValue, byCv, partialCv, byFactory, partialFactory, java, javaCv)
      ...
      val named = Seq("bin2_13", "full2_13_18", "patch2_13_18", "fixedfoo", "part2_13", "one2_13",
        ...
        "byValue2", "byCv2", "partialCv3_3", "byFactory2", "partialFactory3_3", "java", "javaCv")
